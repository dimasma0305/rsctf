...
async fn upgrades_the_real_schema_and_is_idempotent() {
    ...
    .await
    .unwrap();
    // This fixture intentionally stops at m0091. Seal through the historical
    // reconciliation columns that existed at that cutover instead of calling
    // the current helper, which requires m0168's generation columns.
    let sealed_rows = sqlx::query(
        r#"UPDATE "SuspicionReconciliationState"
            ...
            WHERE game_id = 9
              ...
              AND (SELECT end_time_utc + interval '1 second' <= clock_timestamp()
                     FROM "Games" WHERE id = 9)"#,
    )
    .execute(&pool)
    .await
    .unwrap()
...
include!("m0089_cheat_evidence_ledger_online_upgrade_tests.rs");
