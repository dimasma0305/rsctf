...
CREATE TABLE IF NOT EXISTS "SolveReceiptAudit" (
    ...
    outcome TEXT NOT NULL CHECK (outcome IN ('Consumed', 'Expired')),
    consumed_submission_id INTEGER,
    issued_at_utc TIMESTAMPTZ NOT NULL,
    archived_at_utc TIMESTAMPTZ NOT NULL DEFAULT clock_timestamp(),
    CHECK ((outcome = 'Consumed') = (consumed_submission_id IS NOT NULL))
);
CREATE UNIQUE INDEX IF NOT EXISTS ux_solve_receipt_audit_attempt
    ON "SolveReceiptAudit" (attempt_hash);
...
INSERT INTO "SolveReceiptAudit"
    (receipt_id, game_id, challenge_id, participation_id, user_id, variant_id,
     issuer_identity, attempt_hash, token_hash, outcome,
     consumed_submission_id, issued_at_utc, archived_at_utc)
SELECT id, game_id, challenge_id, participation_id, user_id, variant_id,
       issuer_identity, attempt_hash, token_hash, 'Consumed', consumed_submission_id,
...
mod tests {
    ...
    fn receipts_are_idempotent_bounded_and_do_not_block_parent_deletion() {
        ...
        assert!(UP_SQL.contains("ux_solve_receipt_audit_attempt"));
        assert!(UP_SQL.contains("outcome IN ('Consumed', 'Expired')"));
        assert!(UP_SQL.contains("ix_solve_receipt_audit_retention"));
