...
CREATE INDEX IF NOT EXISTS ix_game_notice_outbox_expiry
    ON "GameNoticeOutbox" (delivered_at_utc, id)
    WHERE delivered_at_utc IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_game_notice_operations_expiry
    ON "GameNoticeOperations" (completed_at_utc, game_id, operation_id)
...
mod tests {
    ...
    fn notice_intents_and_delivery_are_durable_and_bounded() {
        ...
        assert!(UP_SQL.contains("WHERE delivered_at_utc IS NOT NULL"));
        assert!(UP_SQL.contains("WHERE completed_at_utc IS NOT NULL"));
    }
