...
fn client_message(text: &str) -> ClientMessageDisposition {
    ...
    for payload in text.split_terminator(RS) {
        ...
        match message.message_type {
            6 if serde_json::from_str::<SignalRPing>(payload)
                .is_ok_and(|ping| ping.message_type == 6) => {}
            7 if serde_json::from_str::<SignalRClose<'_>>(payload).is_ok_and(|close| {
                let _ = close.allow_reconnect;
                close.message_type == 7 && close.error.is_none_or(|error| error.len() <= 256)
            }) =>
            {
                disposition = ClientMessageDisposition::Close;
            }
...
mod tests {
    ...
    fn read_only_signalr_rejects_invocations_and_binary_sized_controls() {
        ...
        assert_eq!(
            client_message("{\"type\":6,\"extra\":true}\u{1e}"),
            ClientMessageDisposition::Unsupported
        );
        assert_eq!(
