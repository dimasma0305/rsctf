...
fn proxy_policies_are_append_only_bounded_buckets() {
    assert!(redis_key(Policy::ProxyOpen, "partition").starts_with("rl:tb:17:"));
    assert!(redis_key(Policy::ProxySourceOpen, "partition").starts_with("rl:tb:18:"));
    assert!(redis_key(Policy::ProxyTraffic, "partition").starts_with("rl:tb:19:"));
    assert!(matches!(
