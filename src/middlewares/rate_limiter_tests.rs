...
#[test]
fn event_vpn_mint_budgets_are_appended_and_partitioned() {
    assert_eq!(
...
fn managed_api_lookup_admission_is_appended_and_source_partitioned() {
    assert_eq!(MANAGED_API_AUTH_CONCURRENCY, 8);
    assert!(MANAGED_API_AUTH_QUERY_TIMEOUT <= Duration::from_secs(2));
    assert_eq!(
