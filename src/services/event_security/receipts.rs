...
async fn lock_receipt_attempt(
    transaction: &mut sqlx::Transaction<'_, sqlx::Postgres>,
    receipt_id: Uuid,
) -> AppResult<()> {
    // Issuance, submission, and expiry archival take this lock before touching
    // the active row. That closes the cross-table uniqueness gap while an
    // attempt moves from SolveReceipts into SolveReceiptAudit.
    sqlx::query(r#"SELECT pg_advisory_xact_lock(hashtextextended($1::text, 827431))"#)
        .bind(receipt_id)
...
pub async fn issue_solve_receipt(
    ...
    let mut transaction = st
        ...
        .await
        .map_err(|error| AppError::internal(error.to_string()))?;
    lock_receipt_attempt(&mut transaction, receipt_id).await?;
    let archived = sqlx::query_scalar::<_, bool>(
        r#"SELECT EXISTS(
               SELECT 1 FROM "SolveReceiptAudit"
                WHERE receipt_id = $1 OR attempt_hash = $2
           )"#,
    )
    .bind(receipt_id)
    ...
    if archived {
        return Err(AppError::conflict(
            "Solve receipt attempt is already expired or consumed",
        ));
    }
...
pub async fn validate_receipt_for_submission(
    ...
    lock_receipt_attempt(transaction, claims.receipt_id).await?;
        ...
        r#"WITH archived AS (
               INSERT INTO "SolveReceiptAudit"
                    (receipt_id, game_id, challenge_id, participation_id, user_id,
                     ...
                     outcome, consumed_submission_id, issued_at_utc, archived_at_utc)
               SELECT id, game_id, challenge_id, participation_id, user_id,
                      variant_id, issuer_identity, attempt_hash, token_hash,
                      'Consumed', $2, issued_at_utc, clock_timestamp()
    ...
    sqlx::query(
        r#"WITH expired AS (
               SELECT receipt_id FROM "SolveReceiptAudit"
                WHERE archived_at_utc < clock_timestamp() - ($1 * interval '1 day')
             ORDER BY archived_at_utc, receipt_id
...
async fn archive_expired_solve_receipts(pool: &sqlx::PgPool) -> Result<u64, sqlx::Error> {
    let mut transaction = pool.begin().await?;
    // The lock CTE is referenced by the archive INSERT, so every selected
    // attempt is serialized with issuance/submission before its active secret
    // is removed. A retained attempt hash prevents semantic reuse afterward.
    let result = sqlx::query(
        r#"WITH candidates AS (
             ...
             ORDER BY expires_at_utc, id
               ...
               SELECT id,
                      pg_advisory_xact_lock(hashtextextended(id::text, 827431))
                 FROM candidates
             ORDER BY id
           ), archived AS (
               INSERT INTO "SolveReceiptAudit"
                    (receipt_id, game_id, challenge_id, participation_id, user_id,
                     variant_id, issuer_identity, attempt_hash, token_hash,
                     outcome, consumed_submission_id, issued_at_utc, archived_at_utc)
               SELECT receipt.id, receipt.game_id, receipt.challenge_id,
                      receipt.participation_id, receipt.user_id, receipt.variant_id,
                      receipt.issuer_identity, receipt.attempt_hash, receipt.token_hash,
                      'Expired', NULL, receipt.issued_at_utc, clock_timestamp()
                 FROM "SolveReceipts" receipt
                 JOIN locked ON locked.id = receipt.id
                WHERE receipt.expires_at_utc < clock_timestamp()
               ON CONFLICT DO NOTHING
               RETURNING receipt_id
           )
           DELETE FROM "SolveReceipts" receipt USING archived
    ...
    tokio::spawn(async move {
        ...
        loop {
            tokio::select! {
                ...
                _ = interval.tick() => {
                    if let Err(error) = archive_expired_solve_receipts(st.pg()).await {
...
mod tests {
    ...
    fn verifier_attempts_derive_stable_opaque_identifiers() {
        ...
        assert!(issuance.contains("lock_receipt_attempt"));
        assert!(source.contains("'Expired', NULL"));
        assert!(source.contains("SolveReceiptAudit"));
    }
    ...
    async fn receipt_audit_retention_is_bounded_and_preserves_recent_rows() {
        ...
        sqlx::raw_sql(
            r#"CREATE TABLE "SolveReceiptAudit" (
               ...
               CREATE INDEX ix_solve_receipt_audit_retention
                   ON "SolveReceiptAudit" (archived_at_utc, receipt_id);"#,
        )
        .execute(&pool)
        .await
        .unwrap();
        for _ in 0..501 {
            sqlx::query(
                r#"INSERT INTO "SolveReceiptAudit" (receipt_id, archived_at_utc)
