...
/// Authorization reconciliation is deliberately one bounded workload across
/// ordinary proxy, SSH, and BYOC transports. A stalled pool must not let each
/// route build its own independent queue of lease queries.
const MAX_CONCURRENT_AUTHORIZATION_QUERIES: usize = 64;
static AUTHORIZATION_QUERY_WORK: tokio::sync::Semaphore =
    tokio::sync::Semaphore::const_new(MAX_CONCURRENT_AUTHORIZATION_QUERIES);

pub(crate) fn try_query_permit() -> Option<tokio::sync::SemaphorePermit<'static>> {
    AUTHORIZATION_QUERY_WORK.try_acquire().ok()
}
