//! Fair, bounded recovery for Flag Egress cursor assignments that lost the
...
pub(super) const CURSOR_LOCK_NAMESPACE: i32 = 1_195_722_073;
...
pub(super) const ASSIGN_PENDING_SQL: &str = r#"
    WITH pending AS MATERIALIZED (
      SELECT queue.event_id
        FROM "FlagEgressFeedPending" queue
        JOIN "FlagEgressEvents" event ON event.id = queue.event_id
       WHERE queue.game_id = $1
       ORDER BY queue.event_id
       LIMIT $2
       FOR UPDATE OF queue, event SKIP LOCKED
    ), assigned AS (
      ...
      DELETE FROM "FlagEgressFeedPending" queue
       USING pending
...
async fn reconcile_pending_once(
    ...
    for game_id in game_ids {
        ...
        .await?;
        super::publish_messages(&st.events, messages);
    }
    Ok(assigned_count)
}

/// Reconcile rows whose commit-time cursor attempt lost a race. Every pass and
/// game batch is bounded, target rows are selected with `SKIP LOCKED`, and the
/// per-game fence is non-blocking, so active control replicas cannot convoy
/// proxy writers or one another.
                ...
                _ = ticker.tick() => {
                    match reconcile_pending_once(&state, &mut last_game_id).await {
                        Ok(count) if count > 0 => tracing::debug!(
                            count,
                            "reconciled pending flag-egress feed cursor(s)"
                        ),
                        Ok(_) => {}
                        Err(error) => tracing::warn!(
                            %error,
                            "flag-egress feed cursor reconciliation failed"
                        ),
                    }
                }
            }
