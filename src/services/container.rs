...
pub use spec::{game_kind_for_challenge, ContainerInfo, ContainerResourceLimits, ContainerSpec};
...
impl ContainerManager for DockerContainerManager {
    ...
    async fn snapshot_changes(&self, id: &str) -> AppResult<Vec<FileChange>> {
        ...
        // JSON response. The narrow daemon client cancels at a fixed byte cap
        // before deserializing, then this mapping applies the entry cap.
        let changes =
            docker_change_api::container_changes(self.endpoint.as_deref(), canonical_id).await?;
        Ok(map_docker_changes_bounded(changes))
    }
