...
/// read. The instance's exact flag row is carried separately as the immutable
/// metadata revision for the scan.
...
pub(super) async fn build_egress_scan(
    ...
    let flag = match EGRESS_METADATA_CACHE.get(&key) {
        ...
        None => {
            ...
            // Bound unique cache-miss leaders before registering a flight. A
            // caller denied here fails closed (the proxy still works; only
            // best-effort telemetry is skipped) without growing the in-flight
            // key map or the PostgreSQL queue.
            let permit = EGRESS_METADATA_QUERIES.try_acquire().ok()?;
            EGRESS_METADATA_SINGLE_FLIGHT
                .run_with_timeout(
                    &flight_key,
                    EGRESS_METADATA_QUERY_DEADLINE,
                    move || async move {
                        // The single-flight leader is detached so cancellation
                        // of its request cannot duplicate the cache fill. Move
                        // the permit into that leader for the same reason.
                        let _permit = permit;
                        ...
                        .bind(i32::try_from(MAX_SCANNED_FLAG_BYTES).unwrap_or(127))
                        .fetch_optional(&pool)
                        .await
                        .ok()??;
                        crate::utils::flag_policy::validate_normal(&flag).ok()?;
...
mod tests {
    ...
    fn cache_key(flag_id: i32) -> EgressMetadataKey {
        EgressMetadataKey {
            game_id: 1,
            participation_id: 2,
            challenge_id: 3,
            container_id: Uuid::nil(),
            revision: EgressMetadataRevision { flag_id },
        }
    }
    ...
    fn metadata_cache_is_bounded_and_revision_keyed() {
        ...
        cache.store(cache_key(11), Arc::from(&b"flag{one}"[..]));
        assert_eq!(&*cache.get(&cache_key(11)).unwrap(), b"flag{one}");
        assert!(cache.get(&cache_key(12)).is_none());

        cache.store(cache_key(12), Arc::from(&b"flag{two}"[..]));
        cache.store(cache_key(13), Arc::from(&b"flag{three}"[..]));
    ...
    fn many_proxy_sessions_keep_a_fixed_per_session_overlap_bound() {
        let flag = vec![b'x'; MAX_SCANNED_FLAG_BYTES];
        let matchers: Vec<_> = (0..4_096)
            .map(|_| RollingFlagMatcher::new(&flag).unwrap())
            .collect();
        assert!(matchers
            .iter()
            .all(|matcher| matcher.max_overlap < MAX_SCANNED_FLAG_BYTES));
        assert_eq!(
            matchers
                .iter()
                .map(|matcher| matcher.max_overlap)
                .sum::<usize>(),
            4_096 * (MAX_SCANNED_FLAG_BYTES - 1)
        );
    }
}
