...
async fn snapshot_after_checkout(
    ...
    let snapshot: AppResult<SnapshotResult> = async {
        ...
        {
            Ok(()) => {}
            Err(AppError::Conflict(_)) => return Ok(SnapshotResult::Retry),
            Err(AppError::NotFound(_)) => return Ok(SnapshotResult::Skip),
        ...
        let Some(manifest) =
            locate_owned_manifest(checkout, expected_binding_id, source_yaml_path.as_deref()).await
        else {
            tracing::warn!(
...
mod queue_contract_tests {
    ...
    fn push_queue_is_durable_coalescing_batched_and_recoverable() {
        ...
        assert!(source.contains("acquire_additional"));
