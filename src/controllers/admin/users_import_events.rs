...
mod tests {
    ...
    async fn event_enrollment_is_accepted_complete_and_idempotent() {
        ...
        let team_locked_by_default: bool =
            sqlx::query_scalar(r#"SELECT locked FROM "Teams" WHERE id = 10"#)
                .fetch_one(&mut connection)
                .await
                .unwrap();
        assert!(!team_locked_by_default);
        sqlx::query(
        ...
        .await
        .unwrap();
        let team_locked_when_enabled: bool =
            sqlx::query_scalar(r#"SELECT locked FROM "Teams" WHERE id = 10"#)
                .fetch_one(&mut connection)
                .await
                .unwrap();
        ...
        assert!(team_locked_when_enabled);
