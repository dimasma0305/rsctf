...
test('monitor history uses one fixed-rate bounded request per iteration', () => {
  ...
  for (const range of ['count=0', 'count=1', 'count=100', 'count=10000', "encodeURIComponent('%_')", 'longSearch']) {
...
test('both live history routes use named heavy-query admission', () => {
  for (const path of ['/api/game/{id}/events', '/api/game/{id}/submissions']) {
