...
test("lifecycle manifests survive failures and explicit audit retention", () => {
  assert.equal(
    shouldRetainLifecycleManifest({ completed: false, cleanupVerified: true }),
    true,
  );
  assert.equal(
    shouldRetainLifecycleManifest({ completed: true, cleanupVerified: false }),
    true,
  );
  assert.equal(
    shouldRetainLifecycleManifest({
...
test("idempotent fetch retry is single-shot and transport-only", async () => {
  ...
  const retried = await fetchWithBoundedTransportRetry(
    async () => {
      calls += 1;
      if (calls === 1) {
  ...
  calls = 0;
  const serverError = await fetchWithBoundedTransportRetry(
    async () => {
      calls += 1;
      return { status: 500 };
    },
...
test("repository cleanup reschedules only the exact disposable binding fixture", () => {
  ...
  assert.match(
    statement,
    /challenge\.source_yaml_path NOT LIKE 'binding\/7\/%'/,
  );
...
test("server runtime identity binds every role to one image and live binary", () => {
  ...
  const containerIds = Object.fromEntries(
    names.map((name, index) => [name, `${index + 1}`.repeat(64)]),
  );
  const fakeDockerSnapshot =
    ({
      ...
      configured = Object.fromEntries(
        names.map((name) => [name, `rsctf:test-${name}`]),
      ),
      mounts = {},
    } = {}) =>
    (args) => {
      ...
      return {
        status: 0,
        stdout: `${binarySha}  /usr/local/bin/rsctf\n`,
        stderr: "",
      };
    };
  const fakeDocker = fakeDockerSnapshot();
  assert.deepEqual(inspectUniformServerRuntimeIdentity(names, fakeDocker), {
    imageId: image,
    binarySha256: binary,
    containers: [
      {
        ...
        configuredImage: "rsctf:test-web-1",
        imageId: image,
        binarySha256: binary,
        startedAt,
        restartCount: 0,
      ...
      },
      {
        ...
        configuredImage: "rsctf:test-web-2",
        imageId: image,
        binarySha256: binary,
        startedAt,
        restartCount: 0,
      ...
      },
      {
        ...
        configuredImage: "rsctf:test-control",
        imageId: image,
        binarySha256: binary,
        startedAt,
        restartCount: 0,
  ...
  assert.throws(
    () =>
      inspectUniformServerRuntimeIdentity(["web", "control"], (args) => {
        ...
        return {
          status: 0,
          stdout: `${binary}  /usr/local/bin/rsctf\n`,
          stderr: "",
        };
      }),
  ...
  assert.throws(
    () =>
      inspectUniformServerRuntimeIdentity(["web", "control"], (args) => {
        ...
        return {
          status: 0,
          stdout: `${hash.repeat(64)}  /usr/local/bin/rsctf\n`,
          stderr: "",
        };
      }),
            ...
            stdout: JSON.stringify([
              {
                ...
                Mounts: [
                  {
                    Type: "bind",
                    Source: "/tmp/debug-rsctf",
                    Destination: "/usr/local/bin/rsctf",
                  },
                ],
              },
            ]),
  ...
  const before = inspectUniformServerRuntimeIdentity(
    names,
    fakeDockerSnapshot(),
  );
  const stableAfter = inspectUnchangedServerRuntimeIdentity(
    before,
    names,
    fakeDockerSnapshot(),
  );
  ...
  const replacements = [
    {
      ...
      docker: fakeDockerSnapshot({
        ids: { ...containerIds, "web-1": "f".repeat(64) },
      }),
    ...
    },
    {
      ...
      docker: fakeDockerSnapshot({
        starts: {
          ...Object.fromEntries(names.map((name) => [name, startedAt])),
          "web-1": "2026-07-20T12:35:00Z",
        },
      }),
...
function repositoryRouterSources() {
  ...
  return {
    ...
    workers: readFileSync(
      join(REPOSITORY, "src/controllers/workers.rs"),
      "utf8",
    ),
...
function sampleBody(kind, status) {
  switch (kind) {
    ...
    case "realtime-metrics":
      return {
        websocket: {
          ...
          inboundFrames: 0,
          inboundBytes: 0,
          inboundQuotaRejections: 0,
          protocolRejections: 0,
          quotaCloses: 0,
          protocolCloses: 0,
          invalidHandshakeCloses: 0,
          idleTimeoutCloses: 0,
          authorizationCloses: 0,
          feedResyncCloses: 0,
        },
        fanout: {
          activeTargetQueues: 0,
          laggedReceivers: 0,
          rejectedEvents: 0,
          distributedDrops: 0,
          distributedLossGeneration: 0,
          subscriberGaps: 0,
        },
      };
    case "game-writeups":
    ...
    case "user":
      return { userId: worker.id, role: "User" };
    case "instance-stats":
      return {
        cpuPercent: 1.5,
    ...
    case "bulk-rebuild":
      return { enqueued: 0, skipped: 0, messages: [] };
    case "prune":
      return { removed: 0, messages: [] };
    case "build":
      return { id: 1, challengeId: 2, status: "Queued" };
    case "build-images":
      return [
        {
    ...
    case "error":
      return { title: "Manual round advance is disabled", status };
    case "workers":
      return [worker];
    case "worker":
      return worker;
    ...
    case "object":
      return {};
    default:
      throw new Error(`missing test sample for ${kind}`);
  }
}
...
test("catalog covers all 78 HTTP operations and keeps SignalR as separate surfaces", () => {
  ...
  assert.deepEqual(
    {
      ...
      path: enroll.path,
      auth: enroll.auth,
      surface: enroll.surface,
    },
    {
      method: "POST",
      path: "/api/workers/enroll",
  ...
  assert.ok(
    ADMIN_SIGNALR_SURFACES.every(({ id }) => !ADMIN_OPERATION_IDS.includes(id)),
  );
});
...
test("read-load subset is GET-only, non-destructive, and includes web and control surfaces", () => {
  ...
  assert.ok(
    ADMIN_READ_OPERATIONS.every(
      ({ method, poll, mutation }) => method === "GET" && poll && !mutation,
    ),
  );
  assert.deepEqual(
    new Set(ADMIN_READ_OPERATIONS.map(({ surface }) => surface)),
    new Set(["web", "control"]),
  );
  assert.ok(
    !ADMIN_READ_OPERATIONS.some(({ id }) => id === "admin_writeups_download"),
  );
});

test("fixed-rate admin load uses one bounded instance batch and never a per-row stats poll", () => {
  const instancePolls = ADMIN_READ_OPERATIONS.filter(({ path }) =>
    path.startsWith("/api/admin/instances"),
  );
  assert.deepEqual(
    instancePolls.map(({ id }) => id),
    ["admin_instances_get"],
  );
  assert.equal(
    instancePolls[0].query,
    "count=25&skip=0&includeRuntimeStats=true",
  );
  assert.equal(
    ADMIN_OPERATIONS.find(({ id }) => id === "admin_instance_stats_get").poll,
    false,
  );
  assert.equal(
    ADMIN_OPERATIONS.find(
      ({ id }) => id === "admin_instance_filter_options_get",
    ).poll,
    false,
  );
});
...
test("owned image fixture inventory binds canonical tags to exact reference state", () => {
  const fixtures = [
    {
      title: "Delete image",
      imageRef: "docker.io/rsctf/7/delete-image:latest",
    },
  ...
  assert.equal(
    assertBuildImageFixtureInventory(records, fixtures, { referenced: true })
      .fixtures.length,
    2,
  );
  assert.throws(
    () =>
      assertBuildImageFixtureInventory(records, fixtures, {
        referenced: false,
      }),
    /reference state/,
  );
  assert.throws(
    () =>
      assertBuildImageFixtureInventory([records[0]], fixtures, {
        referenced: true,
      }),
    /appears 0 times/,
  );
...
test("owned image acceptance uses application import/build/delete flows without fixture ownership seeding", () => {
  ...
  const fixtures = readFileSync(
    join(REPOSITORY, "tests/load/admin-fixtures.mjs"),
    "utf8",
  );
  ...
  assert.doesNotMatch(
    source,
    /docker\(\[\s*['"](?:image\s*,\s*)?tag|['"]container['"]\s*,\s*['"]commit['"]/,
  );
  assert.doesNotMatch(
    source,
    /(?:INSERT\s+INTO|UPDATE|DELETE\s+FROM)\s+['"`]BuildImageOwnerships/i,
  );
});
...
test("container acceptance emulates an external image prune and requires one automatic repair", () => {
  ...
  assert.match(
    drill,
    /docker\(\['container', 'inspect', runtimeRepairContainerId\]\)/,
  );
});
...
test("cleanup acceptance requires two identical all-zero snapshots", () => {
  ...
  assert.throws(
    () => assertStableZeroResidualSnapshots([{ users: 0 }]),
    /exactly two/,
  );
  ...
  assert.ok(
    gate.indexOf("setTimeout(resolve, delayMs)") <
      gate.indexOf("exactResidualSnapshot()"),
...
test("read-origin matrix covers every live read on every eligible replica exactly once", () => {
  ...
  const matrix = buildAdminReadOriginMatrix(
    context,
    webOrigins,
    controlOrigins,
  );
  ...
  assert.equal(
    new Set(
      matrix.map(
        ({ operation, selectedOrigin }) => `${operation.id}|${selectedOrigin}`,
      ),
    ).size,
    matrix.length,
  );
...
test("router-source regression catches a future uncovered operation and a removed route", () => {
  ...
  assert.throws(
    () =>
      assertRouterCoverage({
        ...sources,
        admin: sources.admin.map((source) =>
          source.replace('.route("/api/admin/MyIp", get(my_ip))', ""),
        ),
      }),
...
test("coverage accounting rejects omissions, duplicates, and unknown operations", () => {
  ...
  assert.throws(
    () => assertCompleteCoverage(ADMIN_OPERATION_IDS.slice(1)),
    /missing: admin_my_ip_get/,
  );
  assert.throws(
    () =>
      assertCompleteCoverage([...ADMIN_OPERATION_IDS, ADMIN_OPERATION_IDS[0]]),
    /duplicate/,
  );
  assert.throws(
    () => assertCompleteCoverage([...ADMIN_OPERATION_IDS, "admin_unknown"]),
    /unknown: admin_unknown/,
  );

  const allSurfaces = [
    ...ADMIN_OPERATION_IDS,
    ...ADMIN_SIGNALR_SURFACES.map(({ id }) => id),
  ];
  assert.deepEqual(
...
test("destructive lifecycle safety guard requires an exact disposable topology acknowledgement", () => {
  const env = {
    ...
    CONFIRM_ADMIN_WEB_TARGETS:
      '["http://127.0.0.1:58081","http://127.0.0.1:58082"]',
  ...
  assert.throws(
    () => assertSafeAdminTarget({ ...env, ADMIN_LIFECYCLE_DISPOSABLE: "0" }),
    /DISPOSABLE=1/,
  );
  assert.throws(
    () =>
      assertSafeAdminTarget({
        ...env,
        CONFIRM_ADMIN_TARGET: "http://127.0.0.1:1",
      }),
  ...
  assert.throws(
    () =>
      assertSafeAdminTarget({
        ...env,
        WEB_TARGETS: `${env.TARGET},${env.TARGET}`,
      }),
    /distinct/,
  );
  assert.throws(
    () =>
      assertSafeAdminTarget({
        ...env,
        WEB_TARGETS: `${env.TARGET},http://127.0.0.1:58082`,
      }),
    /TARGET must be distinct/,
  );
  assert.throws(
    () => assertSafeAdminTarget({ ...env, CONTROL_TARGET: env.TARGET }),
    /CONTROL_TARGET/,
  );
  assert.throws(
    () =>
      assertSafeAdminTarget({
        ...env,
        CONTROL_TARGET: "http://127.0.0.1:58081",
      }),
    /CONTROL_TARGET/,
  );
  assert.throws(
  ...
  assert.throws(
    () =>
      assertSafeAdminTarget({
        ...env,
        CONFIRM_ADMIN_WEB_TARGETS:
          '["http://127.0.0.1:58082","http://127.0.0.1:58081"]',
      }),
    /CONFIRM_ADMIN_WEB_TARGETS/,
  );
  assert.throws(
  ...
  assert.throws(
    () =>
      assertSafeAdminTarget({
        ...env,
        CONFIRM_ADMIN_CONTROL_TARGET: "http://127.0.0.1:58084",
      }),
    /CONFIRM_ADMIN_CONTROL_TARGET/,
  );
  const remote = {
    ...env,
    TARGET: "https://192.0.2.20",
    CONFIRM_ADMIN_TARGET: "https://192.0.2.20",
  };
  assert.throws(
    () => assertSafeAdminTarget(remote),
    /ALLOW_REMOTE_ADMIN_LIFECYCLE/,
  );
  assert.equal(
    assertSafeAdminTarget({
      ...remote,
      ALLOW_REMOTE_ADMIN_LIFECYCLE: remote.TARGET,
    }).target,
    remote.TARGET,
  );
});
...
test("destructive backing services require the same marker and Compose project before SQL", () => {
  ...
  assert.throws(
    () =>
      assertDisposableComposeTopology({
        ...
        redis: {
          ...topology.redis,
          environment: ["RSCTF_ADMIN_LIFECYCLE_MARKER=another-stack"],
        },
      }),
  ...
  const main = orchestrator.slice(
    orchestrator.indexOf("async function main()"),
  );
  assert.ok(
    main.indexOf("assertDisposableRuntimeMarker(targets);") <
  ...
  assert.match(
    orchestrator,
    /runtimeIdentity\.after = inspectUnchangedServerRuntimeIdentity/,
  );
  assert.match(
    orchestrator,
    /originalServerRuntimeLogTargets\(startingRuntimeIdentity\)/,
  );
  assert.match(
    orchestrator,
    /countContainerFatalLogs\(containerId, runStartedAt\)/,
  );
...
test("disposable started-game fallback is exact, blob-safe, and rejects forged namespaces", async () => {
  ...
  process.env.RSCTF_JWT_SECRET =
    previousSecret || "admin-fixture-unit-test-secret";
  try {
    const { disposableAdminGameCleanupSql } =
      await import("../admin-fixtures.mjs");
    const cleanup = disposableAdminGameCleanupSql(42, "admabc123");
    assert.match(
      cleanup,
      /id=42 AND title='ADMIN-LIFECYCLE-admabc123' FOR UPDATE/,
    );
    ...
    assert.throws(
      () => disposableAdminGameCleanupSql(0, "admabc123"),
      /positive integer/,
    );
    assert.throws(
      () => disposableAdminGameCleanupSql(42, "ADMIN"),
      /invalid disposable admin namespace/,
    );
    assert.throws(
      () => disposableAdminGameCleanupSql(42, "admabc' OR TRUE--"),
      /invalid disposable admin namespace/,
    );
  } finally {
...
test("operation paths bind and encode disposable context without leaving placeholders", () => {
  ...
  assert.throws(
    () => resolveOperationPath("admin_user_get", {}),
    /requires admin context userId/,
  );
  assert.throws(
    () => resolveOperationPath("not-an-operation", {}),
...
test("response validation rejects malformed pages, leaked credential caching, and wrong manual-advance status", () => {
  assert.equal(
    validateAdminResponse("admin_users_get", {
      status: 200,
      body: { data: [], total: 0, length: 1 },
    }),
    false,
  );
  assert.equal(
  ...
  const realtimeMetrics = sampleBody("realtime-metrics", 200);
  assert.equal(
    validateAdminResponse("admin_realtime_metrics_get", {
      status: 200,
      body: {
        ...realtimeMetrics,
        websocket: { ...realtimeMetrics.websocket, inboundFrames: -1 },
      },
    }),
    false,
  );
  assert.equal(
    validateAdminResponse("admin_realtime_metrics_get", {
      ...
      body: {
        ...realtimeMetrics,
        fanout: { ...realtimeMetrics.fanout, perGameQueues: {} },
      },
    }),
    false,
  );
  ...
  for (const operationId of [
    ...
    assert.equal(
      validateAdminResponse(operationId, {
...
test("stable replica projections remove only request-local and sampled volatility", () => {
  ...
  assert.notDeepEqual(
    stableReplicaProjection("admin_workers_get", firstWorker),
    stableReplicaProjection("admin_workers_get", [
      { ...secondWorker[0], administrativeState: "Disabled" },
    ]),
  );
  assert.deepEqual(
    stableReplicaProjection("admin_my_ip_get", {
      detectedIp: "198.51.100.1",
      rawConnectionIp: "10.0.0.1",
      forwardedFor: "198.51.100.1",
      proxyTrusted: true,
      trustedNetworks: ["10.0.0.0/8", "127.0.0.0/8"],
    }),
    stableReplicaProjection("admin_my_ip_get", {
      detectedIp: "198.51.100.2",
      rawConnectionIp: "10.0.0.2",
      forwardedFor: "198.51.100.2",
      proxyTrusted: true,
      trustedNetworks: ["127.0.0.0/8", "10.0.0.0/8"],
    }),
  );
});
...
test("orchestrator holds a database lease and exercises every privileged authorization class", () => {
  ...
  assert.match(
    lifecycleSource,
    /invalid worker enrollment returned[\s\S]*expected 401/,
  );
  ...
  assert.match(
    lifecycleSource,
    /await call\([\s\S]*?'GET',[\s\S]*?'\/api\/admin\/realtime\/metrics',[\s\S]*?'\/api\/admin\/realtime\/metrics'/,
  );
...
test("cleanup removes build history for every disposable game, including image fixtures", () => {
  ...
  assert.match(
    cleanup,
    /DELETE FROM "BuildRecords" WHERE game_id=ANY\(\$\{ownedGameIds\}\)/,
  );
  assert.match(
    cleanup,
