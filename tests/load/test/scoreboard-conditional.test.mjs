...
const scenario = readFileSync(
  new URL("../k6/scoreboard-conditional.js", import.meta.url),
  "utf8",
);
const runner = readFileSync(
  new URL("../scoreboard-conditional.mjs", import.meta.url),
  "utf8",
);
const standardRoute = readFileSync(
  new URL("../../../src/controllers/game/scoreboard.rs", import.meta.url),
  "utf8",
);
const kothRoute = readFileSync(
  new URL("../../../src/controllers/game/koth/scoreboard.rs", import.meta.url),
  "utf8",
);
...
test("runner uses disposable credentials, samples CPU/RAM, and removes token artifacts", () => {
  ...
  assert.match(
    runner,
    /writeFileSync\(tokenFile,\s*JSON\.stringify\(tokens\),\s*\{ mode: 0o600 \}\)/,
  );
  assert.match(runner, /docker',\s*\n\s*\['stats', '--no-stream'/);
  ...
  assert.match(
    runner,
    /rmSync\(tokenDirectory,\s*\{ recursive: true, force: true \}\)/,
  );
});
...
test("conditional responses remain behind the existing authorization and lifecycle gates", () => {
  ...
  const standardConditional = standardHandler.indexOf(
    "scoreboard_encoding::scoped_response",
  );
  assert.match(standardHandler, /headers:\s*HeaderMap/);
  assert.ok(
    standardHandler.indexOf("g.hidden && !is_monitor") < standardConditional,
  );
  assert.ok(
    standardHandler.indexOf("Utc::now() < g.start_time_utc") <
      standardConditional,
  );

  const kothHandler = kothRoute.slice(
    kothRoute.indexOf("pub async fn scoreboard("),
  );
  const kothConditional = kothHandler.indexOf(
    "scoreboard_encoding::scoped_response",
  );
