...
function controllerSources() {
  return rustFiles(join(REPOSITORY, "src/controllers")).map((path) =>
...
const context = Object.freeze({
  controlJobId: "00000000-0000-4000-8000-000000000010",
  controlJobOperationId: "00000000-0000-4000-8000-000000000011",
  ...
  kothGameId: 13,
  kothChallengeId: 36,
  kothObserverOperationId: "00000000-0000-4000-8000-000000000001",
});

function objectBody(kind) {
  switch (kind) {
    case "message":
      return { title: "", status: 200 };
    case "string":
    case "private-string":
      return "fixture";
    case "number":
    ...
    case "audit-archive":
      return new Uint8Array([0x50, 0x4b, 0x03, 0x04]);
    case "tar":
      return new Uint8Array([0x1f, 0x8b]);
    case "import":
      return { imported: 1, updated: 0, failed: 0, messages: [] };
    case "flag-status":
      return "Success";
    case "inspector":
      return { containerGuid: "0123456789abcdef" };
    case "container":
      return { id: "018f3c6a-d79b-7cc0-8f68-8fdbad0f57bb" };
    case "rollout":
      return {
        matched: 1,
        updated: 1,
    ...
    case "scoring-pause":
      return { scoringPaused: true };
    case "toggle":
      return { isEnabled: false };
    case "snapshot-changes":
      return { snapshotAvailable: true, changes: [] };
    case "snapshot-diff":
      return { added: [], removed: [] };
    case "post":
      return { id: "post-id", title: "Post" };
    case "game":
      return { id: 1, title: "Game" };
    case "challenge":
      return { id: 1, title: "Challenge" };
    case "notice":
      return { id: 1, values: ["notice"], time: Date.now() };
    case "division":
      return { id: 1, name: "Open" };
    case "audit":
      return { archiveAvailable: true, files: [], previews: {} };
    case "build":
      return {
        buildStatus: "Success",
    ...
    case "build-status":
      return {
        ...
        archiveVersion: "a".repeat(64),
      };
    case "build-status-list":
      return [{ challengeId: 1, buildStatus: "Success" }];
    case "control-job":
      return {
    ...
    case "ad-state":
      return { challenges: [], teams: [], scoringPaused: false };
    case "ad-engines":
      return {
    ...
    case "ad-live":
      return { services: [], scoringPaused: false, serverTime: Date.now() };
    case "service-file":
      return { path: "/etc/hostname", containerRunning: true };
    case "review-analytics":
      return { total: 0, likes: 0, dislikes: 0, topLiked: [], topDisliked: [] };
    case "koth-state":
      return { hills: [], teams: [] };
    case "koth-receipts":
      return { challengeId: 1, cycleNumber: 1, receipts: [] };
    case "koth-recovery":
      return { challengeId: 1, cycleNumber: 1, resetPhase: "Active" };
    case "koth-observer":
      return {
        challengeId: 1,
        revision: 1,
  ...
  return {
    ...
    headers:
      operation.responseKind === "tar"
        ...
        : operation.responseKind === "audit-archive"
          ? {
              "content-type": "application/zip",
              "cache-control": "private, no-store",
              "content-disposition":
                'attachment; filename="fixture-source.zip"',
            }
          : operation.responseKind === "import-job"
            ? { "cache-control": "no-store" }
            : ["koth-observer-secret", "koth-observer-mutation"].includes(
...
test("control-plane edit routes retain exact production paths and distinct fixture parameters", () => {
  const expected = new Map([
    [
      ...
      [
        ...
        { operationId: "controlJobOperationId" },
      ],
    ],
    [
      "edit_control_job_get",
      ["GET", "/api/edit/jobs/{jobId}", { jobId: "controlJobId" }],
    ],
    [
      "edit_control_job_cancel",
      ["POST", "/api/edit/jobs/{jobId}", { jobId: "controlJobId" }],
    ],
    [
    ...
    [
      ...
      [
        ...
        { id: "gameId", cId: "archiveChallengeId" },
      ],
    ],
    [
  ...
  for (const [id, [method, path, params]] of expected) {
    const operation = EDIT_OPERATIONS.find((candidate) => candidate.id === id);
    assert.deepEqual(
      operation && {
  ...
  const sourceRoutes = new Set(
    parseEditRouterOperations(controllerSources()).map(
      ({ method, path }) => `${method} ${path}`,
    ),
  );
  for (const [, [method, path]] of expected)
    assert.ok(sourceRoutes.has(`${method} ${path}`));
});
...
test("source drift catches routes added outside controllers/edit and removed catalog routes", () => {
  ...
  assert.throws(
    () =>
      assertEditRouterCoverage([
        ...sources,
        '.route("/api/edit/future", get(future))',
      ]),
    /uncovered: GET \/api\/edit\/future/,
  );
  const withoutPosts = sources.map((source) =>
    source.replace('.route("/api/edit/posts", post(add_post))', ""),
  );
  assert.throws(
    () => assertEditRouterCoverage(withoutPosts),
    /missing: POST \/api\/edit\/posts/,
  );
});
...
test("Admin probes for game-scoped operations require a manager of that exact game", () => {
  const managedGameIds = new Set([
    context.gameId,
    context.adGameId,
    context.kothGameId,
  ]);
  let scopedOperations = 0;
  for (const operation of EDIT_OPERATIONS.filter(
    ({ auth }) => auth === "admin",
  )) {
    ...
    scopedOperations += 1;
    const otherGames = new Set(
      [...managedGameIds].filter((gameId) => gameId !== expectedGameId),
    );
    assert.throws(
  ...
  assert.ok(
    scopedOperations > 0,
    "catalog unexpectedly has no game-scoped Admin operation",
  );
  assert.throws(
    () =>
      assertEditAdminProbeScope(
...
test("every declared response contract has an accepting and rejecting unit sample", () => {
  for (const operation of EDIT_OPERATIONS) {
    assert.equal(
      validateEditResponse(operation, sampleResponse(operation)),
      true,
      operation.id,
    );
    assert.throws(
...
test("coverage accounting rejects missing, duplicate, and unknown operation ids", () => {
  ...
  assert.throws(
    ...
    /missing: edit_control_job_by_operation_get/,
  );
  assert.throws(
    () =>
      assertCompleteEditCoverage([
        ...EDIT_OPERATION_IDS,
        EDIT_OPERATION_IDS[0],
      ]),
    /duplicate/,
  );
  assert.throws(
    () => assertCompleteEditCoverage([...EDIT_OPERATION_IDS, "future"]),
    /unknown: future/,
  );
});
...
test("the disposable orchestrator has one explicit positive call for every catalog id", () => {
  ...
  assert.equal(
    ...
    invoked.length,
    "positive calls must not hide duplicate coverage",
  );
});
...
test("pending challenge fixtures obey the one-manifest public submission boundary", () => {
  ...
  assert.match(
    source,
    /await multipartRequest\([\s\S]*second single-manifest user challenge submission/,
  );
  assert.doesNotMatch(
    source,
...
test("organizer cleanup derives fixture-owned mutable tags without mutating the ownership ledger", () => {
  ...
  assert.doesNotMatch(
    fixtures,
    /FROM\s+['"`]BuildImageOwnerships['"`]\s+WHERE\s+game_id/i,
  );
...
test("failed edit acceptance retains its manifest after successful cleanup", () => {
  ...
  assert.match(
    source,
    /inspectUniformServerRuntimeIdentity\(serverContainers\)/,
  );
  assert.match(
    source,
...
test("edit cleanup is stable and GitHub import is fenced before and after", () => {
  ...
  const stabilityGate = cleanup.slice(
    cleanup.indexOf("async function assertStableExactEditCleanup()"),
  );
  assert.ok(
    stabilityGate.indexOf("setTimeout(resolve, delayMs)") <
      stabilityGate.indexOf("editResidualSnapshot()"),
  ...
  const request = source.indexOf(
    "await call('edit_challenge_import_github'",
    before,
  );
  ...
  assert.ok(
    before >= 0 && before < request && request < after,
    "GitHub ref fence must bracket the import",
  );
  assert.match(
    source,
...
test("inspector acceptance keeps challenge eligibility while simulating an offline service", () => {
  ...
  assert.match(
    prepare,
    /docker\(\['stop', '--time', '2', adRuntimeBeforeRestart\]\)/,
  );
  assert.match(prepare, /await exerciseInspector\(\)/);
  assert.match(prepare, /docker\(\['start', adRuntimeBeforeRestart\]\)/);
  assert.ok(
    prepare.indexOf("isEnabled === true") <
      prepare.indexOf("await exerciseInspector()"),
  );
});
...
test("A&D and KotH create fixtures immediately prove every supplied engine setting persisted", () => {
  ...
  assert.ok(
    adPrepare.indexOf("saveRecovery()") <
      adPrepare.indexOf("assertPersistedGameSettings("),
  );
  assert.ok(
    kothPrepare.indexOf("saveRecovery()") <
      kothPrepare.indexOf("assertPersistedGameSettings("),
  );
  assert.ok(
    adPrepare.indexOf("mutateContainerFilesystem(") <
      adPrepare.indexOf(
        "A.setGameSchedule(context.adGameId, liveStart, liveEnd)",
      ),
  ...
  assert.ok(
    kothPrepare.indexOf("discoverManagedKothHill(") <
      kothPrepare.indexOf(
        "A.setGameSchedule(context.kothGameId, liveStart, liveEnd)",
      ),
...
test("game import acceptance reads back portable semantics and proves late-failure rollback", () => {
  ...
  assert.ok(
    destructive.indexOf("call('edit_game_export'") <
      destructive.indexOf("call('edit_game_admin_delete'"),
    "manager delegation must remain live through the manager-authorized export",
  );
  assert.match(
    fixtures,
    /challengeConfigs:\s*\[\s*\{ challengeId, permissions: 15 \},\s*\{ challengeId, permissions: 15 \}/,
  );
  assert.match(
    fixtures,
...
test("KotH fixture uses public provisioning and proves exact durable and Docker ownership", () => {
  ...
  assert.match(
    fixtures,
    /cycle\.replacement_container_id=target\.container_id/,
  );
  ...
  assert.match(
    fixtures,
    /labels\['rsctf\.operation'\] === owner\.operationOwner/,
  );
...
test("clone acceptance verifies template semantics and rejects inherited live ownership", () => {
  ...
  assert.ok(
    positives.indexOf("saveRecovery()") <
      positives.indexOf("assertSemanticGameClone("),
  );
  ...
  assert.match(
    fixtures,
    /test_container_id IS NOT NULL OR shared_container_id IS NOT NULL/,
  );
  assert.match(
    fixtures,
    /clone changed \$\{flagMismatch\} static flag bindings/,
  );
});
...
test("A&D override acceptance changes authoritative evidence and proves both invalidation layers", () => {
  ...
  assert.match(
    positives,
    /newStatus: overrideTarget\.label, note: overrideNote/,
  );
  assert.match(
    positives,
    /FROM \"AdCheckResults\" WHERE id=\$\{context\.checkId\}/,
  );
  ...
  for (const key of [
    ...
    assert.ok(
      positives.includes(key),
      `missing cache invalidation evidence for ${key}`,
    );
  assert.match(
    positives,
...
test("KotH recovery acceptance faults a scoped durable phase and proves receipt/runtime convergence", () => {
  ...
  assert.match(
    positives,
    /game\.title=\$\{sqlLiteral\(titleFor\(tags\.koth\)\)\}/,
  );
  assert.match(
    positives,
    /challenge\.title=\$\{sqlLiteral\(`edit-koth-\$\{runKey\}`\)\}/,
  );
  assert.match(
    positives,
    /docker\(\['stop', '--time', '2', oldHill\.backendId\]\)/,
  );
  assert.ok(
  ...
  for (const phase of [
    ...
    assert.ok(
      positives.includes(`'${phase}'`),
      `missing ${phase} recovery receipt assertion`,
    );
  assert.match(
    positives,
