...
const k6SummaryPath =
  process.env.EDIT_SUMMARY_JSON ||
  `/tmp/rsctf-edit-lifecycle-${runKey}-k6.json`;
const githubRepository =
  process.env.EDIT_GITHUB_REPOSITORY ||
  "https://github.com/dimasma0305/rsctf-challenges.git";
const githubRef = String(process.env.EDIT_GITHUB_REF || "main").trim();
const githubSubpath =
  process.env.EDIT_GITHUB_SUBPATH || "challenges/Jeopardy/Misc/static-handout";
const githubExpectedCommit = String(
  process.env.EDIT_GITHUB_EXPECTED_COMMIT || "",
)
  .trim()
  .toLowerCase();
if (!/^[a-f0-9]{40}$/.test(githubExpectedCommit)) {
  throw new Error(
    "EDIT_GITHUB_EXPECTED_COMMIT must be a full 40-character Git commit",
  );
}
...
const operationById = new Map(
  EDIT_OPERATIONS.map((operation) => [operation.id, operation]),
);
...
function responseBody(response, operation) {
  if (["zip", "tar", "audit-archive"].includes(operation.responseKind))
...
async function call(
  ...
  if (covered.has(id))
    throw new Error(`${id} was already positively exercised`);
  ...
  const request = ({ timeoutMs = 180_000 } = {}) =>
    rawRequest(operation.method, path, {
      baseUrl,
      jwt,
...
function isTransientKothRecoveryConflict(response) {
  ...
  return (
    title === "replacement container is still transitioning" ||
    title === "checker exit 2" ||
    title === "checker timed out"
  );
}

async function uncatalogued(
  method,
  path,
  { body, headers, jwt = A.adminJwt(), expected = 200 } = {},
) {
...
function seedLifecycleReadFixtures() {
  context.controlJobId = randomUUID();
  context.controlJobOperationId = randomUUID();
  const fingerprint = "a".repeat(64);
  sql(
    `WITH job AS (` +
      `INSERT INTO "ControlPlaneJobs" ` +
      `(id,kind,scope_key,game_id,challenge_id,operation_id,fingerprint,input,` +
      `status,progress_current,progress_total,result,finished_at_utc) VALUES (` +
      `${sqlLiteral(context.controlJobId)}::uuid,'ChallengeBuild',` +
      `${sqlLiteral(`edit-lifecycle:${runKey}`)},${context.gameId},${context.archiveChallengeId},` +
      `${sqlLiteral(context.controlJobOperationId)}::uuid,${sqlLiteral(fingerprint)},` +
      `'{}'::jsonb,2,1,1,'{"fixture":true}'::jsonb,clock_timestamp()) RETURNING id` +
      `) INSERT INTO "ControlPlaneJobOperations" (operation_id,job_id) ` +
      `SELECT ${sqlLiteral(context.controlJobOperationId)}::uuid,id FROM job`,
  );

  context.importJobId = randomUUID();
  const importResult = JSON.stringify({
    imported: 1,
    updated: 0,
    skipped: 0,
    failed: 0,
  ...
  sql(
    `INSERT INTO "ChallengeImportJobs" ` +
      `(id,game_id,actor_user_id,operation_id,source_kind,import_policy,source_key,` +
      `source_staged,status,result) VALUES (` +
      `${sqlLiteral(context.importJobId)}::uuid,${context.gameId},` +
      `${sqlLiteral(identities.managerUserId)}::uuid,${sqlLiteral(randomUUID())}::uuid,` +
      `0,0,${sqlLiteral(`edit-lifecycle:${runKey}`)},FALSE,2,` +
      `${sqlLiteral(importResult)}::jsonb)`,
  );
  saveRecovery();
}
...
async function authorizationMatrix() {
  ...
  for (const [index, operation] of EDIT_OPERATIONS.entries()) {
    ...
    requireCondition(
      missing.status === 401,
      `${operation.id} accepted missing credentials (${missing.status})`,
    );
    missingChecks += 1;
  ...
  console.log(
    `  ✓ ${missingChecks} missing-token and ${privilegeChecks} least-privilege checks`,
  );
}
...
function ensureLocalImage(reference) {
  ...
  if (inspected.status !== 0) {
    ...
    requireCondition(
      pulled.status === 0,
      `pull ${reference}: ${pulled.stderr.trim()}`,
    );
  ...
  requireCondition(
    ...
    `${reference} did not resolve to an immutable image id`,
  );
  return image;
}
...
async function prepareFutureFixture() {
  ...
  const stamp = (id) =>
    sql(
      `SELECT security_stamp FROM "AspNetUsers" WHERE id=${sqlLiteral(id)}::uuid`,
    );
  ...
  identities = {
    ...
    crossManagerUserId,
    crossGameManagerJwt: mintJwt(
      crossManagerUserId,
      stamp(crossManagerUserId),
      1,
    ),
  ...
  await uncatalogued(
    "POST",
    `/api/edit/games/${authorizationGameId}/admins/${crossManagerUserId}`,
  );

  await call("edit_game_admin_add");
  identities.managerGameIds.add(context.gameId);
  const managerList = await call("edit_games_get", {
  ...
  context.deletableChallengeId = await A.createChallenge(context.gameId, {
    title: `edit-delete-${runKey}`,
    category: "Misc",
    type: "StaticAttachment",
  });

  const containerImage = ensureLocalImage(
    process.env.EDIT_CONTAINER_IMAGE || "nginx:alpine",
  );
  context.containerChallengeId = await A.createChallenge(context.gameId, {
    title: `edit-container-${runKey}`,
    category: "Pwn",
  ...
  await A.addFlags(context.gameId, context.containerChallengeId, [
    `flag{edit_container_${runKey}}`,
  ]);

  context.workerChallengeId = await A.createChallenge(context.gameId, {
    title: `edit-workload-${runKey}`,
    category: "Web",
    type: "StaticContainer",
  });
  const workloadSpec = {
  ...
  await A.setChallenge(context.gameId, context.workerChallengeId, {
    workloadSpec,
  });
  ...
  requireCondition(
    Number.isSafeInteger(context.flagId) && context.flagId > 0,
    "flag fixture was not persisted",
  );
  ...
  const pendingApproveArchive = challengeArchive([
    {
      name: `Pending Approve ${runKey}`,
      flag: `flag{pending_approve_${runKey}}`,
    },
  ]);
  ...
  requireCondition(
    submittedApprove.model.imported === 1 &&
      submittedApprove.model.failed === 0,
    "first user submission did not create its pending challenge",
  );
  const pendingRejectArchive = challengeArchive([
    {
      name: `Pending Reject ${runKey}`,
      flag: `flag{pending_reject_${runKey}}`,
    },
  ]);
  ...
  const submittedReject = responseBody(
    submittedRejectResponse,
    submitOperation,
  );
  ...
  context.pendingApproveId = Number(
    sql(
  ...
  context.pendingRejectId = Number(
    sql(
      `SELECT id FROM "GameChallenges" WHERE game_id=${context.gameId} ` +
  ...
  requireCondition(
    imported.model.imported === 1 && imported.model.failed === 0,
    "trusted challenge import failed",
  );
  context.archiveChallengeId = Number(
    sql(
      `SELECT id FROM "GameChallenges" WHERE game_id=${context.gameId} ` +
  ...
  seedLifecycleReadFixtures();

  const githubCommitBefore = resolveRemoteGitRefCommit(
    githubRepository,
    githubRef,
  );
  state.githubImportFence = {
    ...
    commitAfter: null,
    limitation:
      "branch/tag ls-remote fence; the import API does not expose its checked-out commit",
  };
  saveRecovery();
  requireCondition(
  ...
  try {
    const githubCommitAfter = resolveRemoteGitRefCommit(
      githubRepository,
      githubRef,
    );
    state.githubImportFence.commitAfter = githubCommitAfter;
    requireCondition(
      githubCommitAfter === githubCommitBefore &&
        githubCommitAfter === githubExpectedCommit,
  ...
  requireCondition(
    github.model.imported >= 1 && github.model.failed === 0,
    `GitHub import was not successful: ${JSON.stringify(github.model)}`,
  );
  ...
  await call("edit_challenge_attachment", {
    ...
    body: {
      attachmentType: "Remote",
      remoteUrl: `https://example.invalid/${runKey}.txt`,
    },
  });
  ...
  requireCondition(
    /success/i.test(rebuild.model.buildStatus),
    `container rebuild did not succeed: ${JSON.stringify(rebuild.model)}`,
  );
  const rollout = await call("edit_workload_rollout", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    rollout.model.matched === 0 &&
      Object.entries(rollout.model).every(
        ([key, value]) => key === "matched" || value === 0,
      ),
    `zero-instance stateless rollout was not a no-op: ${JSON.stringify(rollout.model)}`,
  );

  const testContainer = await call("edit_test_container_create", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    typeof testContainer.model.id === "string",
    "test-container creation omitted its public id",
  );
  ...
  requireCondition(
    runtime,
    "test-container creation omitted its backend identity",
  );
...
async function prepareAdFixture() {
  ...
  const paused = await call("edit_ad_scoring_pause", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    paused.model.scoringPaused === true,
    "A&D scoring did not pause before fixture configuration",
  );
  context.adChallengeId = await A.createChallenge(context.adGameId, {
    title: `edit-ad-${runKey}`,
    category: "Pwn",
    type: "AttackDefense",
  });
  const checker = A.prepareExactChecker(
    context.adGameId,
    context.adChallengeId,
  );
  const suppliedImage = String(process.env.EDIT_AD_IMAGE || "").trim();
  const image = suppliedImage
    ? ensureLocalImage(suppliedImage)
  ...
  await A.addFlags(context.adGameId, context.adChallengeId, [
    `flag{edit_ad_placeholder_${runKey}}`,
  ]);
  await A.setChallenge(context.adGameId, context.adChallengeId, {
    isEnabled: true,
  });
  ...
  const initiallyProvisioned = disposableAdminGameRuntimeIds(
    context.adGameId,
    tags.ad,
  );
  ...
  const toggle = await call("edit_ad_challenge_toggle", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    toggle.model.isEnabled === false,
    "A&D toggle did not disable the fixture challenge",
  );
  await waitForSql(
  ...
  requireCondition(
    reenabled.json?.isEnabled === true,
    "A&D fixture did not re-enable",
  );
  await uncatalogued(
    "POST",
  ...
  state.runtimeIds.push(
    ...disposableAdminGameRuntimeIds(context.adGameId, tags.ad),
  );
  saveRecovery();
  const stopped = docker(["stop", "--time", "2", adRuntimeBeforeRestart]);
  requireCondition(
    stopped.status === 0,
    `could not stop A&D service for inspector: ${stopped.stderr.trim()}`,
  );
  await exerciseInspector();
  const started = docker(["start", adRuntimeBeforeRestart]);
  requireCondition(
    started.status === 0,
    `could not restore A&D service after inspector: ${started.stderr.trim()}`,
  );
...
async function prepareKothFixture() {
  ...
  context.kothChallengeId = await A.createChallenge(context.kothGameId, {
    title: `edit-koth-${runKey}`,
    category: "Pwn",
    type: "KingOfTheHill",
  });
  const checker = A.prepareKothChecker(
    context.kothGameId,
  ...
  await A.addFlags(context.kothGameId, context.kothChallengeId, [
    `flag{edit_koth_placeholder_${runKey}}`,
  ]);
  ...
  const hill = discoverManagedKothHill(
    context.kothGameId,
    context.kothChallengeId,
  );
...
async function exerciseInspector() {
  try {
    const created = await call("edit_ad_inspector_create", {
      jwt: identities.managerJwt,
    });
    ...
    const backendInspection = docker([
      "container",
      "inspect",
      inspectorBackendId,
    ]);
    requireCondition(
      backendInspection.status === 0,
      "inspector backend is not present after creation",
    );
    const backendRecords = JSON.parse(backendInspection.stdout);
    requireCondition(
      backendRecords.length === 1,
      "inspector backend inspection is ambiguous",
    );
    ...
    requireCondition(
      !networkBindings ||
        Object.values(networkBindings).every(
    ...
    saveRecovery();
    const destroyed = await call("edit_ad_inspector_delete", {
      jwt: identities.managerJwt,
    });
    requireCondition(
      destroyed.response.status === 200,
      "inspector destruction failed",
    );
    requireCondition(
      Number(
        sql(
          `SELECT count(*) FROM "Containers" WHERE id=${sqlLiteral(context.inspectorId)}::uuid`,
        ),
      ) === 0,
    ...
    requireCondition(
      inspection.status !== 0,
      "destroyed inspector runtime is still present",
    );
  } catch (error) {
    ...
    const destroyPath = resolveEditOperationPath(
    ...
    console.error(
      `  ✗ A&D inspector lifecycle is not implemented truthfully: ${error.message}`,
    );
  }
}
...
async function positiveReadAndMutationSurface() {
  ...
  const controlById = await call("edit_control_job_get", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    controlById.model.id === context.controlJobId,
    "control-job lookup returned the wrong durable identity",
  );
  const controlByOperation = await call("edit_control_job_by_operation_get", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    controlByOperation.model.id === context.controlJobId &&
      controlByOperation.model.operationId === context.controlJobOperationId,
    "operation recovery did not resolve the fixture control job",
  );
  const cancelledControl = await call("edit_control_job_cancel", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    cancelledControl.model.id === context.controlJobId &&
      cancelledControl.model.status === "Succeeded",
    "terminal control-job cancellation retry did not preserve the authoritative result",
  );
  const importJob = await call("edit_challenge_import_job_get", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    importJob.model.jobId === context.importJobId &&
      importJob.model.status === "Succeeded",
    "challenge-import recovery returned the wrong terminal fixture",
  );
  ...
  requireCondition(
    /^[a-f0-9]{64}$/.test(salt.model),
    "game hash salt is not a SHA-256 value",
  );
  const variants = await call("edit_variants_get", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    Array.isArray(variants.model),
    "variant ledger response is not an array",
  );
  ...
  requireCondition(
    admins.model.some((user) => user.userId === context.managerUserId),
    "game manager list omitted delegated user",
  );

  const reviews = await call("edit_reviews_get", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    Array.isArray(reviews.model.data),
    "review list is not paged",
  );
  const analytics = await call("edit_reviews_analytics_get", {
    jwt: identities.managerJwt,
  });
  requireCondition(
  ...
  const pending = await call("edit_pending_challenges_get", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    pending.model.some(
      (challenge) => challenge.id === context.pendingApproveId,
    ) &&
      pending.model.some(
        (challenge) => challenge.id === context.pendingRejectId,
      ),
    "pending challenge queue omitted submitted fixtures",
  );
  const challenges = await call("edit_challenges_get", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    challenges.model.some((challenge) => challenge.id === context.challengeId),
    "challenge list omitted primary fixture",
  );
  const challenge = await call("edit_challenge_get", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    challenge.model.id === context.challengeId,
    "challenge detail returned the wrong fixture",
  );
  const audit = await call("edit_challenge_audit_meta", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    audit.model.archiveAvailable === true && audit.model.files.length > 0,
    "archive audit metadata is not materialized",
  );
  const auditArchive = await call("edit_challenge_audit_archive", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    auditArchive.model.length > 4,
    "retained-source archive is empty",
  );
  const buildStatus = await call("edit_challenge_build_status_get", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    buildStatus.model.challengeId === context.archiveChallengeId &&
      buildStatus.model.archiveAvailable === true,
    "challenge build status omitted the retained-source fixture",
  );
  const buildStatuses = await call("edit_challenge_build_statuses_get", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    buildStatuses.model.some(
      ({ challengeId }) => challengeId === context.archiveChallengeId,
    ),
    "challenge build-status inventory omitted the retained-source fixture",
  );
  ...
  requireCondition(
    notice.model.values[0].includes("updated"),
    "notice update did not persist",
  );
  ...
  requireCondition(
    division.model.name === `edit-updated-${runKey}`,
    "division update did not persist",
  );

  const adState = await call("edit_ad_state", { jwt: identities.managerJwt });
  requireCondition(
    adState.model.challenges.some(
      (challenge) => challenge.challengeId === context.adChallengeId,
    ),
    "A&D state omitted fixture challenge",
  );
  const adEngines = await call("edit_ad_engines", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    adEngines.model.hasAttackDefense === true &&
      adEngines.model.hasKoth === false,
  ...
  requireCondition(
    adLive.model.services.some(
      (service) => service.adTeamServiceId === context.serviceId,
    ),
    "A&D live projection omitted the fixture service",
  );
  const serviceFile = await call("edit_ad_service_file", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    serviceFile.model.containerRunning === true,
    "A&D service file did not inspect a live container",
  );
  const changes = await call("edit_ad_snapshot_changes", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    changes.model.changes.some((change) =>
      change.path.includes("/tmp/rsctf-edit"),
    ),
    "snapshot changes omitted real filesystem drift",
  );
  const diff = await call("edit_ad_snapshot_diff", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    diff.model.added.some((path) => path.includes("/tmp/rsctf-edit")),
    "snapshot diff omitted real added path",
  );
  const points = await call("edit_ad_snapshots_get", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    points.model.length > 0,
    "snapshot history did not expose live drift point",
  );
  const snapshot = await call("edit_ad_snapshot_download", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    snapshot.model.length >= 2,
    "compressed snapshot archive is empty",
  );
  ...
  requireCondition(
    oldRuntime.status !== 0,
    "A&D restart left the retired runtime present",
  );

  await A.setAdScoringPaused(context.adGameId, false);
  context.checkId = Number(
    await waitForSql(
  ...
  await waitForSql(
    ...
    (value) => {
      try {
        ...
        return (
          Number(checkerTiming.expected) >= 3 &&
          checkerTiming.observed === checkerTiming.expected
        );
      } catch {
        return false;
      }
    ...
    {
      timeoutMs: 180_000,
      label: "complete independently scheduled A&D checker field",
    },
  );
  requireCondition(
    Number(checkerTiming.minimumMs) >=
      Number(checkerTiming.graceSeconds) * 1000 - 50,
  ...
  await uncatalogued("PUT", `/api/edit/games/${context.adGameId}`, {
    jwt: identities.managerJwt,
  ...
  await waitForSql(
    ...
    (value) => {
      try {
        ...
        return (
          Number.isSafeInteger(overrideBefore.id) &&
          overrideBefore.id > 0 &&
  ...
  const expectedOverrideMessage = `[admin override: ${AD_CHECK_STATUS[overrideBefore.status]} -> ${overrideTarget.label}] ${overrideNote}`;
  ...
  for (const key of scoreboardKeys) {
    requireCondition(
      redisRaw(
        ["SET", key, cacheSentinel, "EX", "300"],
        `seed A&D cache key ${key}`,
      ) === "OK",
  ...
  const rollupSuffixBefore = Number(
    sql(
  ...
  requireCondition(
    rollupSuffixBefore >= Number(overrideBefore.rollupSuffixCount) &&
      rollupSuffixBefore > 0,
  ...
  const overrideAfter = JSON.parse(
    sql(
      `SELECT json_build_object(` +
  ...
  const rollupSuffixAfter = Number(
    sql(
      `SELECT count(*) FROM "AdEpochRollups" ` +
        `WHERE game_id=${context.adGameId} AND epoch>=${Number(overrideBefore.epoch)}`,
    ),
  ...
  requireCondition(
    rollupSuffixAfter === 0,
    "A&D override left its affected rollup suffix materialized",
  );
  ...
  const kothState = await call("edit_koth_state", {
    jwt: identities.managerJwt,
  });
  ...
  requireCondition(
    activeHillView.durablePhase === "Active" &&
      activeHillView.resetPhase === "Active",
    `KotH fixture was not active before recovery fault: ${JSON.stringify(activeHillView)}`,
  );
  requireCondition(
    activeHillView.claimSource === "Api" &&
      activeHillView.apiObserverConfigured === true,
  ...
  requireCondition(
    claimToken,
    "KotH API fixture has no exact active capability",
  );
  ...
  const stagedObservation = JSON.parse(
    sql(
      `SELECT json_build_object(` +
  ...
  saveRecovery();
  const receipts = await call("edit_koth_receipts", {
    jwt: identities.managerJwt,
  });
  requireCondition(
    receipts.model.challengeId === context.kothChallengeId,
    "KotH receipt feed returned wrong hill",
  );
  ...
  await A.setAdScoringPaused(context.kothGameId, true);
  const oldHill = discoverManagedKothHill(
    context.kothGameId,
    context.kothChallengeId,
  );
  const cycleBefore = JSON.parse(
    sql(
      `SELECT json_build_object(` +
  ...
  const replacementHill = discoverManagedKothHill(
    context.kothGameId,
    context.kothChallengeId,
  );
  requireCondition(
  ...
  requireCondition(
    cycleAfter.id === cycleBefore.id &&
      ...
      cycleAfter.readinessError === null &&
      cycleAfter.lastError === null &&
      cycleAfter.replacementContainerId === replacementHill.backendId &&
      cycleAfter.targetContainerId === replacementHill.backendId &&
      typeof cycleAfter.targetHost === "string" &&
      cycleAfter.targetHost.length > 0 &&
  ...
  const receiptModel = responseBody(
    receiptResponse,
    operationById.get("edit_koth_receipts"),
  );
  ...
  const recoveryReceiptPhases = new Set(
    recoveryReceipts.map((receipt) => receipt.phase),
  );
  for (const phase of [
    "DestroyPending",
    "CreatePending",
  ...
  const preexistingReceiptIds = new Set(
    state.kothRecovery.preexistingReceiptIds,
  );
  requireCondition(
  ...
  const recoveredState = responseBody(
    stateResponse,
    operationById.get("edit_koth_state"),
  );
...
async function runReadSimulation() {
  ...
  requireCondition(
    status === 0,
    `k6 edit lifecycle failed with exit code ${status}`,
  );
  state.load = {
    skipped: false,
...
async function destructivePositiveSurface() {
  ...
  const revokedObserver =
    revokedObserverResponse.json?.data ?? revokedObserverResponse.json;
  requireCondition(
  ...
  const exported = await call("edit_game_export", {
    jwt: identities.managerJwt,
  });
  ...
  const importedGameRead = await uncatalogued(
    "GET",
    `/api/edit/games/${importedGameId}`,
  );
  const importedGameModel =
    importedGameRead.json?.data ?? importedGameRead.json;
  requireCondition(
    importedGameModel?.id === importedGameId,
    "imported game could not be read back",
  );
  requireCondition(
    importedGameModel.poster == null,
    "imported game exposed a dangling source poster",
  );
  ...
  const importedChallenges =
    importedChallengeRead.json?.data ?? importedChallengeRead.json;
  requireCondition(
    Array.isArray(importedChallenges),
    "imported challenge list could not be read back",
  );
  const importedCheckerSummary = importedChallenges.find(
    ({ title }) => title === transferCheckerTitle,
  );
  requireCondition(
    importedCheckerSummary?.id > 0,
    "imported challenge list omitted the checker sentinel",
  );
  ...
  const importedChecker =
    importedCheckerRead.json?.data ?? importedCheckerRead.json;
  requireCondition(
    importedChecker?.adCheckerImage == null,
    "imported checker sentinel retained executable state",
  );
  requireCondition(
    importedChecker.adAllowEgress === true &&
      importedChecker.adAllowSelfReset === true &&
      importedChecker.adSshRequiresFlag === true &&
      importedChecker.adScoringWeight === 1.1,
  ...
  const rollbackCounts = assertFailedGameImportRolledBack(
    rollbackProbe,
    serverContainers,
  );
  state.importRollback = {
    status: rollbackResponse.status,
    counts: rollbackCounts,
  };
  saveRecovery();
  console.log(
    "  ✓ late-invalid import rolled back game, challenge, attachment, file, flag, and division state",
  );
  ...
  requireCondition(
    deleted.model.id === context.gameId,
    "game delete returned the wrong model",
  );
  state.gameIds = state.gameIds.filter((id) => id !== context.gameId);
  state.futureGameIds = state.futureGameIds.filter(
    (id) => id !== context.gameId,
  );
  saveRecovery();
}
...
async function deleteFutureGame(gameId) {
  ...
  if (title === titleFor(tags.auth)) {
    deleteDisposableAdminGame(gameId, tags.auth, {
      runtimeIds: state.runtimeIds,
    });
    requireCondition(
      Number(sql(`SELECT count(*) FROM "Games" WHERE id=${Number(gameId)}`)) ===
        0,
  ...
  requireCondition(
    Number(sql(`SELECT count(*) FROM "Games" WHERE id=${Number(gameId)}`)) ===
      0,
    `future game ${gameId} survived cleanup`,
  );
}
...
function dockerObjectPresent(kind, identity, label) {
  ...
  if (
    /no such (?:container|image|object)/i.test(
      String(inspected.stderr || inspected.stdout || ""),
    )
  )
    return 0;
  throw new Error(
    `could not audit ${label} ${identity}: ${String(inspected.stderr || inspected.stdout || "").trim()}`,
  );
}
...
function editResidualSnapshot() {
  ...
  const postIds = [
    ...new Set([...state.postIds, context.postId].filter(Boolean).map(String)),
  ];
  ...
  return Object.freeze({
    games: gameIds.length
      ? Number(
          sql(
            `SELECT count(*) FROM "Games" WHERE id IN (${gameIds.join(",")})`,
          ),
        )
      : 0,
    buildRecords: gameIds.length
      ? Number(
          sql(
            `SELECT count(*) FROM "BuildRecords" WHERE game_id IN (${gameIds.join(",")})`,
          ),
        )
      : 0,
    gameNamespace: Number(
      sql(
        `SELECT count(*) FROM "Games" WHERE strpos(title, ${sqlLiteral(runKey)}) > 0`,
      ),
    ),
    ...
    posts: postIds.length
      ? Number(
          sql(
            `SELECT count(*) FROM "Posts" WHERE id IN (${postIds.map(sqlLiteral).join(",")})`,
          ),
        )
      : 0,
    runtimeContainers: [...new Set(state.runtimeIds.filter(Boolean))].reduce(
      (count, id) =>
        count + dockerObjectPresent("container", id, "runtime container"),
      0,
    ),
    checkerDirectories: gameIds.reduce(
      (count, gameId) =>
        count +
        serverContainers.reduce(
          (serverCount, container) =>
            serverCount + checkerDirectoryPresent(container, gameId),
          0,
        ),
      0,
    ),
    cacheKeys: [...new Set(state.cacheKeys || [])].reduce(
      (count, key) =>
        count + Number(redisRaw(["EXISTS", key], `audit cache key ${key}`)),
      0,
    ),
    ownedImages: [...new Set(state.ownedImageRefs || [])].reduce(
      (count, image) =>
        count + dockerObjectPresent("image", image, "owned image"),
      0,
    ),
    failedImportResidue: Object.values(rollbackCounts).reduce(
...
async function assertStableExactEditCleanup() {
  ...
  for (let pass = 0; pass < 2; pass += 1) {
    ...
    for (const [resource, count] of Object.entries(snapshot)) {
      requireCondition(
        count === 0,
        `${resource} cleanup residue is ${count}, expected zero`,
      );
    }
...
function removeOwnedBuildHistory() {
  ...
  requireCondition(
    Number(
      sql(`SELECT count(*) FROM "Games" WHERE id IN (${gameIds.join(",")})`),
    ) === 0,
...
async function cleanup() {
  ...
  await capture("pause A&D scoring", async () => {
    if (
      context.adGameId &&
      Number(sql(`SELECT count(*) FROM "Games" WHERE id=${context.adGameId}`))
    ) {
      await A.setAdScoringPaused(context.adGameId, true);
    }
  });
  await capture("pause KotH scoring", async () => {
    if (
      context.kothGameId &&
      Number(sql(`SELECT count(*) FROM "Games" WHERE id=${context.kothGameId}`))
    ) {
      await A.setAdScoringPaused(context.kothGameId, true);
    }
  });
    ...
    ...new Set(
      [
        ...
        ...(context.adGameId
          ...
          : []),
        ...(context.kothGameId
          ? disposableAdminGameRuntimeIds(context.kothGameId, tags.koth)
          : []),
        ...(context.kothGameId
          ? A.kothContainerIdsForGames([context.kothGameId])
          : []),
      ].filter(Boolean),
    ),
  ];
  ...
  for (const gameId of [context.adGameId, context.kothGameId].filter(Boolean)) {
    await capture(`checker ${gameId}`, async () =>
      removeCheckerDirectory(gameId),
    );
  }
  await capture("exact A&D graph", async () => {
    if (context.adGameId)
      deleteDisposableAdminGame(context.adGameId, tags.ad, { runtimeIds });
  });
  await capture("exact KotH graph", async () => {
    if (context.kothGameId)
      deleteDisposableAdminGame(context.kothGameId, tags.koth, { runtimeIds });
  });

  for (const gameId of [...new Set(state.futureGameIds)]) {
    await capture(`future game ${gameId}`, async () =>
      deleteFutureGame(gameId),
    );
  }
  ...
  for (const postId of state.postIds) {
    await capture(`post ${postId}`, async () => {
      const response = await rawRequest(
        "DELETE",
        `/api/edit/posts/${encodeURIComponent(postId)}`,
        { jwt: A.adminJwt() },
      );
      if (response.status !== 404)
        expectStatus(response, 200, `cleanup post ${postId}`);
    });
  }
...
async function main() {
  ...
  try {
    ...
    state.cleanupFailure = String(
      cleanupError?.stack || cleanupError?.message || cleanupError,
    );
    saveRecovery();
    failure = failure
      ? new AggregateError(
          [failure, cleanupError],
          "edit lifecycle and cleanup both failed",
        )
      : cleanupError;
  }
  try {
    ...
    const fatalLogsByContainer = Object.fromEntries(
      originalServerRuntimeLogTargets(startingRuntimeIdentity).map(
        ({ name, containerId }) => [
          ...
          {
            containerId,
            fatalLineCount: countContainerFatalLogs(
              containerId,
              state.startedAt,
            ),
          },
    ...
    const fatalLogs = Object.values(fatalLogsByContainer).reduce(
      (sum, value) => sum + value.fatalLineCount,
      0,
    );
    requireCondition(
      fatalLogs === 0,
    ...
    state.verificationFailure = String(
    ...
    failure = failure
      ? new AggregateError(
          ...
          "edit lifecycle and verification both failed",
        )
      : verificationError;
  }
  for (const [label, lease] of [
    ...
    try {
      ...
      const wrapped = new Error(
        `${label} lifecycle lease release failed: ${releaseError.message}`,
      );
      ...
      failure = failure
        ? new AggregateError(
            [failure, wrapped],
            "edit lifecycle and lease release both failed",
          )
        : wrapped;
    }
  }
  ...
  state.failure = failure
    ? String(failure.stack || failure.message || failure)
    : null;
  saveRecovery();
