...
export const ADMIN_OPERATIONS = Object.freeze([
  operation("admin_my_ip_get", "GET", "/api/admin/MyIp", {
    poll: true,
    responseKind: "my-ip",
  }),
  operation("admin_config_get", "GET", "/api/admin/config", {
    poll: true,
    responseKind: "config",
  }),
  ...
  operation("admin_logo_delete", "DELETE", "/api/admin/config/logo", {
    mutation: true,
    responseKind: "message",
  }),
  ...
  operation(
    "admin_realtime_metrics_get",
    "GET",
    "/api/admin/realtime/metrics",
    {
      poll: true,
      responseKind: "realtime-metrics",
    },
  ),
  operation(
    "admin_flag_egress_get",
    "GET",
  ...
  operation("admin_users_add", "POST", "/api/admin/users", {
    mutation: true,
    responseKind: "message",
  }),
  operation("admin_users_import", "POST", "/api/admin/users/import", {
    mutation: true,
    responseKind: "import",
  }),
  operation(
  ...
  operation(
    "admin_manager_autocomplete_get",
    "GET",
  ...
  operation(
    "admin_instance_filter_options_get",
    "GET",
  ...
  operation("admin_captcha_test", "POST", "/api/admin/captcha/test", {
    mutation: true,
    responseKind: "message",
  }),
  ...
  operation(
    "admin_event_vpn_override_create",
    "POST",
  ...
  operation(
    "admin_event_vpn_override_revoke",
    "POST",
  ...
  operation(
    "ad_admin_round_advance_rejected",
    "POST",
...
export const ADMIN_SIGNALR_SURFACES = Object.freeze([
  Object.freeze({
    id: "admin_signalr_connect",
    method: "GET",
    path: "/hub/admin",
    surface: "web",
  }),
  Object.freeze({
    id: "admin_signalr_negotiate",
    method: "POST",
    path: "/hub/admin/negotiate",
    surface: "web",
  }),
]);

export const ADMIN_OPERATION_IDS = Object.freeze(
  ADMIN_OPERATIONS.map(({ id }) => id),
);
export const ADMIN_READ_OPERATIONS = Object.freeze(
  ADMIN_OPERATIONS.filter(({ poll }) => poll),
);
...
export function assertExactFailedBuildPruneCandidates(records, expectedId) {
  if (!Array.isArray(records))
    throw new TypeError("build prune inventory must be an array");
  ...
  const active = normalized.filter(
    ({ status }) => status === 3 || status === 5,
  );
  if (active.length) {
    throw new Error(
      `failed-build prune has active candidates: ${JSON.stringify(active)}`,
    );
  }
  ...
  return Object.freeze({
    ...
    candidates: Object.freeze([fixtureId]),
  });
}
...
function validBuildImage(image) {
  return (
    ...
    image.tags.every((tag) => typeof tag === "string" && tag.length > 0) &&
    Number.isSafeInteger(image.sizeBytes) &&
    image.sizeBytes >= 0 &&
    (image.createdUtc === null ||
      image.createdUtc === undefined ||
      Number.isFinite(image.createdUtc)) &&
    (image.lastUsedUtc === null || Number.isFinite(image.lastUsedUtc)) &&
    Number.isFinite(image.retentionExpiresUtc) &&
    typeof image.inUse === "boolean" &&
    typeof image.referenced === "boolean" &&
    Array.isArray(image.referencedBy) &&
    image.referencedBy.every((title) => typeof title === "string") &&
...
function validStorageCleanup(body) {
  return (
    object(body) &&
    Number.isSafeInteger(body.imagesRemoved) &&
    body.imagesRemoved >= 0 &&
    [
      "imageBytesEvicted",
      "cacheBytesReclaimed",
    ...
    typeof body.pressureMode === "boolean" &&
    Array.isArray(body.messages) &&
    body.messages.every((message) => typeof message === "string")
  );
}

export function assertBuildImageFixtureInventory(
  records,
  fixtures,
  { referenced } = {},
) {
  ...
  if (
    ...
    throw new Error(
      "build image fixture assertion requires fixtures and an expected reference state",
    );
  }
  const matches = fixtures.map((fixture) => {
    if (
      ...
      typeof fixture.imageRef !== "string"
    ) {
    ...
    if (matching.length !== 1) {
      throw new Error(
        `${fixture.imageRef} appears ${matching.length} times in owned image inventory`,
      );
    }
    const image = matching[0];
    if (image.referenced !== referenced) {
      throw new Error(
        `${fixture.imageRef} reference state is ${image.referenced}, expected ${referenced}`,
      );
    }
    const ownsTitle = image.referencedBy.includes(fixture.title);
    if (ownsTitle !== referenced) {
      throw new Error(
        `${fixture.imageRef} reference titles do not match ${fixture.title}`,
      );
    }
    if (!referenced && image.referencedBy.length !== 0) {
      throw new Error(
        `${fixture.imageRef} retained unrelated references: ${image.referencedBy.join(", ")}`,
      );
    }
    return Object.freeze({ imageRef: fixture.imageRef, imageId: image.id });
  });
  return Object.freeze({
    fixtures: Object.freeze(matches),
...
export function assertStableZeroResidualSnapshots(snapshots) {
  ...
  const normalized = snapshots.map((snapshot, index) => {
    ...
    for (const [resource, count] of Object.entries(snapshot)) {
      if (!Number.isSafeInteger(count) || count < 0) {
        throw new Error(
          `cleanup pass ${index + 1} has invalid ${resource} count`,
        );
      }
      if (count !== 0) {
        throw new Error(
          `cleanup pass ${index + 1} retained ${resource}: ${count}`,
        );
      }
    }
    return snapshot;
  ...
  return Object.freeze({
    passes: 2,
    resources: Object.keys(normalized[0]).length,
  });
}
...
export function assertCompleteCoverage(recorded, options = {}) {
  ...
  if (duplicates.length || missing.length || (!allowExtra && extra.length)) {
    const details = [
      ...
      !allowExtra && extra.length
        ? `unknown: ${[...new Set(extra)].join(", ")}`
        : "",
    ].filter(Boolean);
    throw new Error(
      `incomplete admin lifecycle coverage (${details.join("; ")})`,
    );
  }
  return Object.freeze({
...
function parseTargetList(raw) {
  ...
  if (text.startsWith("[")) {
    ...
    if (!Array.isArray(parsed))
      throw new Error("web targets JSON must be an array");
...
export function assertSafeAdminTarget(env = {}) {
  if (env.ADMIN_LIFECYCLE_DISPOSABLE !== "1") {
    throw new Error(
      "set ADMIN_LIFECYCLE_DISPOSABLE=1 for the destructive admin lifecycle",
    );
  }
  ...
  if (!loopback && env.ALLOW_REMOTE_ADMIN_LIFECYCLE !== target) {
    throw new Error(
      `remote admin lifecycle requires ALLOW_REMOTE_ADMIN_LIFECYCLE=${target}`,
    );
  }
  const webTargets = parseTargetList(
    env.WEB_TARGETS || env.ADMIN_WEB_TARGETS,
  ).map((value, index) => normalizedHttpUrl(value, `WEB_TARGETS[${index}]`));
  if (webTargets.length < 2 || new Set(webTargets).size !== webTargets.length) {
    throw new Error(
      "WEB_TARGETS must contain at least two distinct direct web-replica origins",
    );
  }
  if (webTargets.includes(target)) {
    throw new Error(
      "TARGET must be distinct from every direct WEB_TARGETS origin",
    );
  }
  const controlTarget = normalizedHttpUrl(
    env.CONTROL_TARGET || "",
    "CONTROL_TARGET",
  );
  if (controlTarget === target || webTargets.includes(controlTarget)) {
    throw new Error(
      "CONTROL_TARGET must be distinct from TARGET and every direct web target",
    );
  }
  return assertAdminOriginAcknowledgements(env, {
    target,
    webTargets,
    controlTarget,
  });
...
export function assertAdminOriginAcknowledgements(
  env,
  { target, webTargets, controlTarget },
) {
  if (confirmationOrigin(env.CONFIRM_ADMIN_TARGET) !== target) {
    throw new Error(
      `set CONFIRM_ADMIN_TARGET=${target} to acknowledge the exact disposable target`,
    );
  }
  const confirmedWebTargets = parseTargetList(
    env.CONFIRM_ADMIN_WEB_TARGETS,
  ).map(confirmationOrigin);
  if (
  ...
  if (confirmationOrigin(env.CONFIRM_ADMIN_CONTROL_TARGET) !== controlTarget) {
    throw new Error(
      `set CONFIRM_ADMIN_CONTROL_TARGET=${controlTarget} to acknowledge the direct control origin`,
    );
  }
  return Object.freeze({
    target,
...
function validateDisposableContainer(resource, label, marker) {
  ...
  if (!project || !service)
    throw new Error(`${label} ${name} is not a Compose service`);
  ...
  if (
    ...
    markerEntries[0] !== `${DISPOSABLE_MARKER_ENV}=${marker}`
  ) {
    throw new Error(
      `${label} ${name} does not carry the one exact disposable marker`,
    );
  }
...
export function assertDisposableComposeTopology({
  marker,
  servers,
  postgres,
  redis,
  ...
  if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]{7,127}$/.test(expected)) {
    throw new Error(
      "ADMIN_LIFECYCLE_STACK_MARKER must name the dedicated server-side disposable marker",
    );
  }
  if (!Array.isArray(servers) || servers.length < 3) {
    throw new Error(
      "declare at least two web replicas and one control replica",
    );
  }
  ...
  if (
    new Set(serverResources.map(({ name }) => name)).size !==
  ...
  if (projects.size !== 1) {
    throw new Error(
      "declared web/control containers are not in one Compose project",
    );
  }
  ...
  for (const [resource, label, expectedService] of [
    ...
    if (
      inspected.project !== composeProject ||
      inspected.service !== expectedService
    ) {
      throw new Error(
...
function directContainerEndpoint(origin, label) {
  ...
  if (
    ...
    throw new Error(
      `${label} must be exactly http://<declared-container-ip>:8080`,
    );
  }
  return Object.freeze({
...
export function assertDirectAdminOriginBindings({
  webTargets,
  controlTarget,
  servers,
} = {}) {
  if (!Array.isArray(webTargets) || !Array.isArray(servers)) {
    throw new Error(
      "direct admin origin binding requires webTargets and inspected servers",
    );
  }
  const declaredWeb = servers.filter(({ service }) => service === "rsctf");
  const declaredControl = servers.filter(
    ({ service }) => service === "rsctf-control",
  );
  if (
    webTargets.length !== declaredWeb.length ||
    declaredControl.length !== 1
  ) {
    throw new Error(
  ...
  const normalizedServers = servers.map((server, index) => {
    ...
    if (
      !name ||
      !Array.isArray(server.networkAddresses) ||
      server.networkAddresses.length === 0
    ) {
      throw new Error(
        `server[${index}] ${name || "<unnamed>"} has no inspectable network IP`,
      );
    }
    return {
      name,
  ...
  const bindings = targets.map((target) => {
    const matches = normalizedServers.filter(({ networkAddresses }) =>
      networkAddresses.has(target.host),
    );
    if (matches.length !== 1) {
      throw new Error(
        `${target.origin} maps to ${matches.length} declared server containers; expected exactly one`,
      );
    }
    ...
    if (boundNames.has(server.name)) {
      throw new Error(
        `direct admin origins map more than once to ${server.name}`,
      );
    }
  ...
  if (boundNames.size !== normalizedServers.length) {
    throw new Error(
      "one or more declared server containers has no exact direct admin origin",
    );
  }
...
export function buildAdminReadOriginMatrix(
  context,
  webOrigins,
  controlOrigins,
) {
  ...
  for (const operation of ADMIN_READ_OPERATIONS) {
    ...
    const origins =
      operation.surface === "control" ? controlOrigins : webOrigins;
...
function privateNoStore(headers) {
  ...
  return (
    /(?:^|,)\s*private\b/i.test(cacheControl) &&
...
const WEBSOCKET_METRIC_KEYS = Object.freeze([
  "startedAtUnixMs",
  "connectionsRejected",
  "inboundFrames",
  "inboundBytes",
  "inboundQuotaRejections",
  "protocolRejections",
  "quotaCloses",
  "protocolCloses",
  "invalidHandshakeCloses",
  "idleTimeoutCloses",
  "authorizationCloses",
  "feedResyncCloses",
]);
const FANOUT_METRIC_KEYS = Object.freeze([
  "activeTargetQueues",
  "laggedReceivers",
  "rejectedEvents",
  "distributedDrops",
  "distributedLossGeneration",
  "subscriberGaps",
]);
const REALTIME_METRIC_KEYS = Object.freeze(["websocket", "fanout"]);
...
function validExactNonnegativeIntegerFields(value, keys) {
  return (
...
function validRealtimeMetrics(body) {
  return (
    hasExactKeys(body, REALTIME_METRIC_KEYS) &&
    validExactNonnegativeIntegerFields(body.websocket, WEBSOCKET_METRIC_KEYS) &&
    validExactNonnegativeIntegerFields(body.fanout, FANOUT_METRIC_KEYS)
  );
}
...
function validMessage(body, status) {
  return (
    object(body) && typeof body.title === "string" && body.status === status
  );
}
...
export function validateAdminResponse(operationId, response) {
  ...
  switch (item.responseKind) {
    ...
    case "message":
      return validMessage(body, Number(response.status));
    case "string":
      return typeof body === "string" && body.length > 0;
    case "private-string":
      return (
        typeof body === "string" && body.length > 0 && privateNoStore(headers)
      );
    case "my-ip":
      return (
        object(body) &&
        typeof body.detectedIp === "string" &&
        typeof body.rawConnectionIp === "string" &&
        typeof body.forwardedFor === "string" &&
        typeof body.proxyTrusted === "boolean" &&
        Array.isArray(body.trustedNetworks)
      );
    case "config":
      return (
        object(body) &&
        object(body.accountPolicy) &&
        object(body.globalConfig) &&
        object(body.containerPolicy) &&
        object(body.proxyTrust)
      );
    case "dashboard":
      return (
        object(body) &&
        ...
        ["userCount", "teamCount", "activeContainerCount"].every(
          (key) =>
            Number.isSafeInteger(body.systemStats[key]) &&
            body.systemStats[key] >= 0,
        )
      );
    ...
    case "zip": {
      ...
      return (
        /application\/zip/i.test(headerValue(headers, "content-type")) &&
    ...
    case "import":
      return (
        object(body) &&
        ["total", "created", "updated", "skipped"].every(
          (key) => Number.isSafeInteger(body[key]) && body[key] >= 0,
        ) &&
    ...
    case "credential-send":
      return (
        object(body) &&
        Number.isSafeInteger(body.sent) &&
        Number.isSafeInteger(body.failed) &&
        Array.isArray(body.results) &&
        body.sent + body.failed === body.results.length &&
        privateNoStore(headers)
      );
    case "user":
      return (
    ...
    case "instance-stats":
      return (
        ...
        [
          ...
          "netTxBytes",
          "sampledAt",
        ].every((key) => number(body[key])) &&
        body.cpuPercent >= 0 &&
        body.memoryUsedBytes >= 0
      );
    case "bulk-rebuild":
      return (
        object(body) &&
        Number.isSafeInteger(body.enqueued) &&
        body.enqueued >= 0 &&
        Number.isSafeInteger(body.skipped) &&
        body.skipped >= 0 &&
        Array.isArray(body.messages)
      );
    case "prune":
    ...
    case "build-images":
      return Array.isArray(body) && body.every(validBuildImage);
    case "build-storage":
      return validBuildStorage(body);
    case "storage-cleanup":
      return validStorageCleanup(body);
    case "repo-scan-result":
      return validRepoScan(body);
    case "repo-binding":
      return (
        object(body) &&
        Number.isSafeInteger(body.id) &&
    ...
    case "ad-service":
      return (
        object(body) &&
        Number.isSafeInteger(body.adTeamServiceId) &&
        Number.isSafeInteger(body.participationId) &&
        Number.isSafeInteger(body.challengeId) &&
    ...
    case "workers":
      return Array.isArray(body) && body.every(validWorker);
    case "worker":
      return validWorker(body);
    case "worker-created":
      return (
        object(body) &&
        validWorker(body.worker) &&
        object(body.enrollment) &&
        typeof body.enrollment.token === "string" &&
        body.enrollment.workerId === body.worker.id &&
        privateNoStore(headers)
      );
    ...
    case "enrollment":
      return (
        object(body) &&
        typeof body.workerId === "string" &&
        typeof body.controlAddress === "string" &&
        typeof body.dataAddress === "string" &&
        typeof body.serverName === "string" &&
        typeof body.certificatePem === "string" &&
        body.certificatePem.length > 0 &&
        typeof body.caPem === "string" &&
        body.caPem.length > 0 &&
        privateNoStore(headers)
      );
    case "object":
...
export function stableReplicaProjection(operationId, body) {
  if (!operationById.has(operationId))
    throw new Error(`unknown admin operation ${operationId}`);
  if (operationId === "admin_my_ip_get") {
    return canonical({
      proxyTrusted: body?.proxyTrusted,
      trustedNetworks: Array.isArray(body?.trustedNetworks)
        ? [...body.trustedNetworks].sort()
        : body?.trustedNetworks,
      detectedIpPresent:
        typeof body?.detectedIp === "string" && body.detectedIp.length > 0,
      rawConnectionIpPresent:
        typeof body?.rawConnectionIp === "string" &&
        body.rawConnectionIp.length > 0,
    });
  }
...
function rustRawStringEnd(source, start) {
  ...
  if (end < 0)
    throw new Error("unterminated Rust raw string while scanning admin routes");
...
function routeCalls(source) {
  ...
  for (let cursor = 0; cursor < source.length; cursor += 1) {
    ...
    if (outerBlockDepth > 0) {
      if (outerChar === "/" && outerNext === "*") {
        outerBlockDepth += 1;
        cursor += 1;
      } else if (outerChar === "*" && outerNext === "/") {
        outerBlockDepth -= 1;
        cursor += 1;
      }
      continue;
...
function parsedRoutes(source) {
  ...
  for (const call of routeCalls(source)) {
    ...
    if (methods.length === 0)
      throw new Error(`could not parse HTTP method for ${path}`);
...
export function parseAxumRouterOperations(source) {
  if (typeof source !== "string")
    throw new TypeError("Axum router source must be a string");
  return Object.freeze(parsedRoutes(source).map(Object.freeze));
}

export function parseAdminRouterOperations(sources) {
  if (!sources || typeof sources !== "object")
    throw new TypeError("router sources must be an object");
  const operations = [];
  for (const key of ["admin", "ad", "workers"]) {
    const chunks =
      typeof sources[key] === "string" ? [sources[key]] : sources[key];
    if (
    ...
    for (const source of chunks) {
      for (const route of parsedRoutes(source)) {
        if (
          ...
          route.path.startsWith("/api/ad/admin/") ||
          route.path === "/api/workers/enroll"
        ) {
  ...
  const hubChunks =
    typeof sources.adminHub === "string"
      ? [sources.adminHub]
      : sources.adminHub;
  const signalR = Array.isArray(hubChunks)
    ? hubChunks
        .flatMap((source) => parsedRoutes(source))
        .filter(
          ({ path }) => path === "/hub/admin" || path.startsWith("/hub/admin/"),
        )
    : [];
  return Object.freeze({
...
export function assertRouterCoverage(sources) {
  ...
  if (
    routes.missing.length ||
    routes.extra.length ||
    actual.size !== parsed.operations.length
  ) {
    throw new Error(
  ...
  if (typeof sources.adminHub === "string" || Array.isArray(sources.adminHub)) {
    ...
    if (
      hubs.missing.length ||
      hubs.extra.length ||
      actualHub.size !== parsed.signalR.length
    ) {
      throw new Error(
