...
const WEB_TARGETS = targetList(
  __ENV.WEB_TARGETS || __ENV.ADMIN_WEB_TARGETS,
  "WEB_TARGETS",
);
...
const MAX_VUS = positiveInteger(
  __ENV.MAX_VUS || Math.max(VUS, RATE * 4),
  "MAX_VUS",
);
const DURATION = __ENV.DURATION || "60s";
const MAX_ADMIN_P95_MS = positiveNumber(
  __ENV.MAX_ADMIN_P95_MS || 1000,
  "MAX_ADMIN_P95_MS",
);
const MAX_HEALTH_P95_MS = positiveNumber(
  __ENV.MAX_HEALTH_P95_MS || 500,
  "MAX_HEALTH_P95_MS",
);
...
if (__ENV.ADMIN_LIFECYCLE_DISPOSABLE !== "1") {
  throw new Error(
    "set ADMIN_LIFECYCLE_DISPOSABLE=1 before sending an admin token",
  );
}
if (
  !isLoopbackOrigin(TARGET) &&
  __ENV.ALLOW_REMOTE_ADMIN_LIFECYCLE !== TARGET
) {
  throw new Error(
...
if (
  WEB_TARGETS.length < 2 ||
  new Set(WEB_TARGETS).size !== WEB_TARGETS.length
) {
  throw new Error(
...
if (RATE > 2) {
  throw new Error(
    "RATE must stay at or below 2 req/s: the 81-request setup matrix shares the 150/min admin quota",
  );
}
...
const eligibleReads = ADMIN_READ_OPERATIONS.map((operation) =>
  Object.freeze({
    operation,
    path: resolveOperationPath(operation, ADMIN_CONTEXT),
  }),
);
...
const endpointTrends = Object.fromEntries(
  ADMIN_READ_OPERATIONS.map((operation) => [
    operation.id,
    new Trend(`${operation.id}_ms`, true),
  ]),
);
...
function origin(value, label) {
  ...
  if (
    !/^https?:\/\/[^/?#]+$/i.test(text) ||
    /[\s@]/.test(text.replace(/^https?:\/\//i, ""))
  ) {
    throw new Error(
      `${label} must be a credential-free HTTP(S) origin without a path`,
    );
  }
  return text;
}
...
function performAdminRead({ operation, path, selectedOrigin }, headers, phase) {
  ...
  if (phase === "matrix") {
    adminMatrixFailure.add(!expected || !valid, {
      operation: operation.id,
      target: selectedOrigin,
    });
    adminMatrixSamples.add(1, {
      operation: operation.id,
      target: selectedOrigin,
    });
    adminMatrixMs.add(response.timings.duration, {
      operation: operation.id,
      target: selectedOrigin,
    });
  } else {
    adminReadMs.add(response.timings.duration, { operation: operation.id });
    endpointTrends[operation.id].add(response.timings.duration, {
      target: selectedOrigin,
    });
  }
...
export function adminReads() {
  ...
  const origins =
    selected.operation.surface === "control" ? controlOrigins : webOrigins;
  const selectedOrigin =
    origins[Math.floor(sequence / eligibleReads.length) % origins.length];
  performAdminRead(
...
export function platformHealth() {
  ...
  const duration = responses.reduce(
    (maximum, response) => Math.max(maximum, response.timings.duration),
    0,
  );
  ...
  check(responses, {
    "all public, direct-replica, and control health endpoints remain valid":
      () => healthy,
  });
}
...
export function adminSignalR() {
  const selectedOrigin =
    webOrigins[exec.scenario.iterationInTest % webOrigins.length];
    ...
    (socket) => {
      ...
      socket.on("message", (message) => {
        if (
          String(message)
            .split("\u001e")
            .some((frame) => frame.trim() === "{}")
        ) {
          handshaken = true;
          signalrHandshakeMs.add(Date.now() - started, {
            target: selectedOrigin,
          });
          socket.close();
        }
