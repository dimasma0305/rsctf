...
try {
  ...
  // Check the painted focus ring, not the frame before the keyboard style is applied.
  await evaluate(`new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)))`)
  ...
  assert.equal(await evaluate(`document.querySelector('#event-catalog-results [data-guide="event-card"] > div').style.getPropertyValue('--event-hue')`), `${(901 * 47 + 186) % 360}deg`, 'Filtering must preserve the event color')
