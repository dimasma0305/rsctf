...
const output = resolve('../visual-audit-output/rework-workspaces')
...
const categories = { Web: challenges.filter((c) => c.category === 'Web'), Misc: challenges.filter((c) => c.category === 'Misc') }
...
try {
  ...
  const pages = ['/admin/dashboard', '/admin/games', '/admin/users', '/admin/settings', '/teams', '/account/profile', '/games/901/challenges', '/games/901/scoreboard']
  ...
  for (const [viewport, width, height] of [['mobile', 390, 844], ['tablet', 768, 1024], ['wide', 1920, 1080]]) {
    await cdp.send('Emulation.setDeviceMetricsOverride', { width, height, deviceScaleFactor: 1, mobile: false })
    for (const path of ['/admin/settings', '/games/901/challenges']) await visit(path, `${viewport}${path.replaceAll('/', '--')}`)
  }
  ...
  for (const path of ['/admin/settings', '/games/901/challenges', '/teams']) await visit(path, `light-id${path.replaceAll('/', '--')}`)
