...
import selectClasses from '@Styles/ScrollSelect.module.css'
...
export const TeamItem: SelectableItemComponent<TeamTrafficModel> = (itemProps) => {
  ...
  return (
    <SelectableItem h={itemHeight} pr={5} {...props}>
      <Group justify="space-between" gap="sm" w="100%" wrap="nowrap" maw="100%">
        <Group justify="left" gap="xs" wrap="nowrap" miw={0} flex={1}>
          <Avatar imageProps={{ loading: 'lazy' }} alt={item.name ?? ''} src={item.avatar} radius="xl" size={30}>
...
export const FileItem: FC<PropsWithItem<FileItemProps, FileRecord>> = (itemProps) => {
  const { item, onDownload, onDelete, onInspect, disabled, t, locale } = itemProps
  ...
  return (
    ...
    >
      <UnstyledButton
        ...
        w="calc(100% - 5rem)"
        disabled={disabled}
        aria-label={`${t('common.label.download')}: ${item.fileName ?? ''}`}
        ...
        {onInspect && (
          <Tooltip label={t('game.label.flow.inspect_action')}>
            <ActionIcon
              variant="subtle"
              color="blue"
              aria-label={t('game.label.flow.inspect_action')}
              disabled={disabled}
