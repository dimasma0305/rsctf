...
test('donations use a feature-gated page instead of the home feed', () => {
  ...
  assert.match(navbar, /getWorkspaceNavigation\(location\.pathname, user, config\.donationsEnabled\)/)
  assert.match(navigation, /canAccessNavigationItem\(item, user, donationsEnabled\)/)
