...
test('capability renewal stays idle when clock readiness arrives after the fallback delay', async (context) => {
  ...
  const sampleDelay = MINIMUM_DELAY_MS + 1_500
  ...
  try {
    ...
    // A browser-ahead calculation used to clamp to one second and reset a
    // freshly issued capability before any trustworthy clock existed.
    await act(async () => context.mock.timers.tick(sampleDelay))
    assert.equal(serverClockTestApi.ready(), false)
    assert.equal(renewals, 0)

    await act(async () => {
      assert.equal(observeServerTime(serverStart + sampleDelay, localStart + sampleDelay), true)
    })
    assert.equal(serverClockTestApi.ready(), true)
    const refreshDelay = 5 * 60_000 - sampleDelay
    await act(async () => context.mock.timers.tick(refreshDelay - 1))
    assert.equal(renewals, 0)
