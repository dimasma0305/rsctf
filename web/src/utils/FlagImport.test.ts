...
test('flag imports bound rows and UTF-8 field bytes before sending', () => {
  ...
  assert.match(localModal, /uploadOperationId\.current \?\?= crypto\.randomUUID\(\)/)
  assert.match(localModal, /flagOperationId\.current \?\?= crypto\.randomUUID\(\)/)
  assert.match(localModal, /\{ filename: uploadFileName, operationId \}/)
  assert.match(localModal, /operationId: flagOperationId\.current/)
