...
test('traffic wire contract uses real numeric timestamps, filters, and pagination', () => {
  ...
  assert.match(api, /interface FlowDetailQuery[\s\S]*?peerIp\?: string/)
  assert.match(api, /gameGetTrafficFlowDetail:[\s\S]*?query: FlowDetailQuery[\s\S]*?query,/)
  assert.match(inspector, /key={`\$\{flow\.connectionPort\}:\$\{flow\.peerIp\}`}/)
  assert.match(inspector, /next\.set\('port',[\s\S]*?next\.set\('flowPeer',/)
  assert.match(detail, /\{ peerIp \}/)
})
