...
async function main() {
	...
	const creep = {
		...
		workerScope: !fp.workerScope || fp.workerScope.lied ? undefined : {
			...
			webglRenderer: (
				(fp.workerScope.gpu && fp.workerScope.gpu.confidence != 'low') ? fp.workerScope.gpu.compressedGPU : undefined
			),
			webglVendor: (
				(fp.workerScope.gpu && fp.workerScope.gpu.confidence != 'low') ? fp.workerScope.webglVendor : undefined
			),
			userAgentData: {
