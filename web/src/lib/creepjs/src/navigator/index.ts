...
export default async function getNavigator(workerScope) {
	try {
		...
		const credibleUserAgent = (
			(typeof window !== 'undefined' && 'chrome' in window) ? navigator.userAgent.includes(navigator.appVersion) : true
		)
		const data = {
			platform: attempt(() => {
			...
			plugins: attempt(() => {
				...
				if (typeof PluginArray === 'undefined' || !(plugins instanceof PluginArray)) {
					return
				}
...
export function navigatorHTML(fp) {
	...
	<div class="col-six${lied ? ' rejected' : ''}">
		...
		<div class="help" title="Navigator.globalPrivacyControl">gpc: ${'' + globalPrivacyControl == 'undefined' ? HTMLNote.UNSUPPORTED : '' + globalPrivacyControl
		}</div>
		<div class="help" title="Navigator.language\nNavigator.languages">lang: ${!blocked[language] ? language : HTMLNote.BLOCKED
		}</div>
		...
		<div class="help" title="Permissions.query()">permissions (${'' + permissionsGranted}): ${!permissions || !permissionsKeys ? HTMLNote.UNSUPPORTED : modal(
			'creep-permissions',
