...
import creepWorkerUrl from '../creep.ts?worker&url'
...
export async function spawnWorker() {
	...
	const getWebglData = () => ask(() => {
		...
		if (!rendererInfo) {
			return {
				webglVendor: contextWebgl.getParameter(contextWebgl.VENDOR),
				webglRenderer: contextWebgl.getParameter(contextWebgl.RENDERER),
			}
		}
		return {
...
export default async function getBestWorkerScope() {
	try {
		...
		const getDedicatedWorker = ({ scriptSource }) => new Promise((resolve) => {
			...
			const dedicatedWorker = ask(() => new Worker(scriptSource, { type: 'module' }))
		...
		const getSharedWorker = ({ scriptSource }) => new Promise((resolve) => {
			...
			const sharedWorker = ask(() => new SharedWorker(scriptSource, { type: 'module' }))
		...
		const getServiceWorker = ({ scriptSource }) => new Promise((resolve) => {
			...
			return navigator.serviceWorker.register(scriptSource, { type: 'module' }).then((registration) => {
		...
		const scriptSource = creepWorkerUrl
...
export function workerScopeHTML(fp) {
	...
	<div class="relative col-six${lied ? ' rejected' : ''}">
		...
		<div class="relative">${confidence ? `<span class="confidence-note">confidence: <span class="scale-up grade-${confidenceGrade}">${confidence}</span></span>` : ''
		}gpu:</div>
